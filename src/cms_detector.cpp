...
void CMSDetector::init_fingerprints() {
  fingerprints = {
      ...
      {"MediaWiki",
       ...
       {"X-Powered-By"},
       {"mediawiki"},
       "/api.php?action=siteinfo&format=json",
       "generator.*?([0-9.]+)",
       "1.41.0"},
      {"vBulletin",
       {"/vb/"},
...
std::string CMSDetector::extract_version(const std::string &content,
...
std::vector<CMSVersion>
CMSDetector::detect_with_version(const std::string &base_url) {
  ...
  for (const auto &fp : fingerprints) {
    ...
    if (detected && !fp.version_path.empty()) {
      ...
      bool outdated = false;
      if (!version.empty() && !fp.latest_version.empty() &&
          fp.latest_version != "N/A") {
        outdated = (version != fp.latest_version);
      }

      results.push_back({fp.name, version.empty() ? "Unknown" : version,
                         fp.latest_version, outdated});
    } else if (detected) {
