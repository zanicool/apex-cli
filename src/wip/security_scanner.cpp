...
SecurityScanner::SecurityScanner(const std::string& target_url) 
    : target_url_(target_url), http_client_(std::make_unique<HttpClient>()) {
}

void SecurityScanner::enable_spa_mode(bool enable) {
    spa_mode_ = enable;
}

void SecurityScanner::set_auth_jwt(const std::string& token) {
    auth_headers_["Authorization"] = "Bearer " + token;
}

void SecurityScanner::set_auth_cookie(const std::string& name, const std::string& value) {
    auth_headers_["Cookie"] = name + "=" + value;
}
...
std::vector<SecurityFinding> SecurityScanner::check_injection() {
    ...
    std::vector<std::string> sqli_payloads = {
        "' OR '1'='1",
        "' OR '1'='1' --",
        "' OR '1'='1' /*",
        "admin'--",
...
std::vector<SecurityFinding> SecurityScanner::check_nextjs_api_routes() {
    ...
    for (const auto& route : api_routes) {
        ...
        if (response.status_code == 200 && 
            response.body.find("unauthorized") == std::string::npos &&
...
std::vector<SecurityFinding> SecurityScanner::check_graphql_introspection() {
    ...
    for (const auto& endpoint : graphql_endpoints_) {
        ...
        if (response.status_code == 200 && 
            response.body.find("__schema") != std::string::npos) {
