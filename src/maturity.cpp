...
int MaturityCalculator::calculate_security_score(const std::vector<Finding>& findings) {
    int critical = std::count_if(findings.begin(), findings.end(),
        [](const Finding& f) { return f.severity == "critical"; });
    int high = std::count_if(findings.begin(), findings.end(),
...
int MaturityCalculator::calculate_reliability_score(const OSINTReport& osint) {
    int critical_leaks = std::count_if(osint.leaks.begin(), osint.leaks.end(),
...
MaturityScore MaturityCalculator::calculate(
    const std::vector<Finding>& findings,
    ...
    score.dimensions["Security"] = {"Security", security, 
        security >= 80 ? "Good" : security >= 60 ? "Fair" : "Poor"};
    score.dimensions["Maintainability"] = {"Maintainability", maintainability,
        maintainability >= 80 ? "Good" : maintainability >= 60 ? "Fair" : "Poor"};
    score.dimensions["Reliability"] = {"Reliability", reliability,
        reliability >= 80 ? "Good" : reliability >= 60 ? "Fair" : "Poor"};
    ...
    if (score.percentage >= 90) score.level = 5;
    else if (score.percentage >= 75) score.level = 4;
    else if (score.percentage >= 60) score.level = 3;
    else if (score.percentage >= 40) score.level = 2;
    else if (score.percentage >= 20) score.level = 1;
    else score.level = 0;
