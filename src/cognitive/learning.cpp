...
std::pair<std::string, double> LearningEngine::best_strategy_for(const std::string& context) const {
  std::string best;
...
std::vector<StrategyRecord> LearningEngine::ranked_strategies(const std::string& context) const {
  std::vector<StrategyRecord> relevant;

  for (const auto& [key, record] : strategy_performance_) {
    if (context.empty() || key.find(context) != std::string::npos || record.context == context) {
  ...
  std::sort(relevant.begin(), relevant.end(),
            [](const StrategyRecord& a, const StrategyRecord& b) { return a.success_rate > b.success_rate; });

  return relevant;
}

double LearningEngine::adjusted_value(const std::string& strategy, const std::string& context, double raw_expected_value) const {
...
LearningMetrics LearningEngine::metrics() const {
  ...
  for (const auto& exp : experiments_) {
    if (exp.produced_finding)
      m.useful_experiments++;
    else
      wasted++;
  ...
  for (const auto& [key, record] : strategy_performance_) {
    ...
    if (record.success_rate > best_rate) {
      best_rate = record.success_rate;
      m.best_strategy = record.strategy + " in " + record.context + " (" + std::to_string((int)(best_rate * 100)) + "%)";
    }
    if (record.success_rate < worst_rate) {
      worst_rate = record.success_rate;
      m.worst_strategy = record.strategy + " in " + record.context + " (" + std::to_string((int)(worst_rate * 100)) + "%)";
    }
  }
...
std::string LearningEngine::summary() const {
  ...
  ss << "  Useful actions: " << m.useful_experiments << "/" << m.total_experiments << " (" << (int)((1.0 - m.wasted_action_rate) * 100)
     << "% efficiency)\n";
  ...
  if (!m.best_strategy.empty()) ss << "  Best strategy: " << m.best_strategy << "\n";
  if (!m.worst_strategy.empty()) ss << "  Worst strategy: " << m.worst_strategy << "\n";
  if (m.total_experiments >= 20)
    ss << "  Improvement: " << (m.improvement_over_baseline > 0 ? "+" : "") << (int)(m.improvement_over_baseline * 100)
       << "% vs baseline\n";
  return ss.str();
}
