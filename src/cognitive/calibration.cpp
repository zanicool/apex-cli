...
void CalibrationEngine::record_prediction(const std::string &hypothesis_id,
...
ReviewResult Reviewer::review(const Finding &finding, const WorldModel &world) const {
  ...
  result.objections.insert(result.objections.end(),
                           evidence_objections.begin(), evidence_objections.end());
  result.objections.insert(result.objections.end(),
                           alternative_objections.begin(), alternative_objections.end());
  result.objections.insert(result.objections.end(),
                           fp_objections.begin(), fp_objections.end());
...
ReviewResult Reviewer::review_hypothesis(const CognitiveHypothesis &hypothesis,
  ...
  if (hypothesis.supporting_beliefs.empty()) {
    result.objections.push_back({
        ...
        "No supporting beliefs — hypothesis is speculative",
  ...
  if (hypothesis.prior_probability > 0.6 && hypothesis.status == "untested") {
    result.objections.push_back({
        hypothesis.statement,
        "High confidence without testing — possible overconfidence",
        "Reduce prior or test immediately",
        0.5});
  }
  ...
  result.adjusted_confidence = std::max(0.1,
      result.original_confidence - total_severity * 0.2);
...
std::vector<Objection> Reviewer::challenge_evidence(const Finding &f) const {
  ...
  if (f.evidence.empty()) {
    objections.push_back({
        f.type,
        "No evidence provided — detection only, not confirmation",
        "Reproduce the finding with differential testing",
        0.7});
  }
  ...
  if ((f.severity == "critical" || f.severity == "high") && f.payload.empty()) {
    objections.push_back({
        f.type,
        "Critical/high finding without a specific payload — how was it triggered?",
        "Provide exact request that demonstrates the vulnerability",
        0.5});
  }
  ...
  if (f.type.find("SQL") != std::string::npos && f.evidence.find("SQL") == std::string::npos &&
    ...
    objections.push_back({
        f.type,
        "SQL injection claimed but no database error in evidence",
        "Test with time-based blind or trigger specific error",
        0.6});
  }
  ...
  if (f.type.find("XSS") != std::string::npos && f.evidence.find("alert") == std::string::npos &&
    ...
    objections.push_back({
        f.type,
        "XSS claimed but payload not confirmed in evidence",
        "Verify exact reflection context (in attribute? in JS? in HTML?)",
        0.5});
  }
...
std::vector<Objection> Reviewer::find_alternatives(const Finding &f,
                                                    const WorldModel &world) const {
  std::vector<Objection> objections;

  // Could the response be a generic error page?
  if (f.evidence.find("<!DOCTYPE") != std::string::npos ||
      f.evidence.find("<HTML>") != std::string::npos) {
    objections.push_back({
        f.type,
        "Evidence contains HTML — possibly a generic error/WAF page, not actual vulnerability indicator",
        "Compare with baseline response for this endpoint",
        0.8});
  }
  ...
  if (f.type.find("Bypass") != std::string::npos || f.type.find("Access") != std::string::npos) {
    objections.push_back({
        f.type,
        "Access control bypass: does status 200 mean actual data access, or just a different error page?",
        "Verify response contains actual protected content, not just a 200 with generic body",
        0.4});
  }
...
std::vector<Objection> Reviewer::check_fp_indicators(const Finding &f) const {
  ...
  if (f.url.find("cloudflare") != std::string::npos ||
      f.evidence.find("Attention Required") != std::string::npos ||
      f.evidence.find("Access Denied") != std::string::npos) {
    objections.push_back({
        f.type,
        "WAF/CDN response detected — this is likely a false positive",
        "Retry with WAF evasion or confirm this is actual application response",
        0.9});
  }
  ...
  if (f.type.find("Time") != std::string::npos && f.type.find("Blind") != std::string::npos) {
    objections.push_back({
        f.type,
        "Time-based detection: network jitter could explain timing difference",
        "Repeat with multiple sleep values (2s, 4s, 6s) — timing should be proportional",
        0.4});
  }
