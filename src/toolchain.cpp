...
std::vector<Tool> detect_tools() {
  const std::vector<std::string> names = {"nuclei", "httpx", "katana", "subfinder",    "ffuf",   "sqlmap",
...
std::vector<std::string> run_katana(const std::string& target, int depth) {
  std::string cmd = "katana -u https://" + target + " -d " + std::to_string(depth) + " -silent -no-color -jc";
...
std::string run_sqlmap(const Finding& finding) {
  ...
  if (!finding.param.empty()) target_url += "?" + finding.param + "=1";
...
std::string run_dalfox(const Finding& finding) {
  ...
  std::string target_url = finding.url;
  if (!finding.param.empty()) target_url += "?" + finding.param + "=FUZZ";
  std::string cmd = "dalfox url '" + target_url + "' --silence 2>/dev/null | head -10";
  return exec_cmd(cmd, 30);
}

std::vector<std::string> run_ffuf(const std::string& base_url, const std::string& wordlist) {
  std::string cmd = "ffuf -u '" + base_url + "/FUZZ' -w " + wordlist + " -mc 200,301,302,403 -s 2>/dev/null";
  std::string out = exec_cmd(cmd, 60);
...
std::vector<Finding> run_nuclei(const std::string& target, const std::string& tags) {
  std::string cmd = "nuclei -u https://" + target + " -tags " + tags + " -severity critical,high,medium -jsonl -silent 2>/dev/null";
