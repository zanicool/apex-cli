...
std::vector<Scanner> register_wordpress_deep_scanners() {
    return {
        {"WP REST API Sensitive Endpoints", [](const Config &cfg, HttpClient &http, const CrawlResult &crawl) -> std::vector<Finding> {
            ...
            for (const auto &base : bases) {
                for (const auto &[ep, desc] : endpoints) {
                    ...
                    if (resp.status_code == 200 && resp.body.size() > 50 &&
                        resp.body.find("\"code\"") == std::string::npos) {
                        // Check if it contains sensitive data
                        bool sensitive = resp.body.find("email") != std::string::npos ||
                                        ...
                                        resp.body.find("customer") != std::string::npos;
                        if (sensitive) {
                            findings.push_back({
                                "WP REST API Data Exposure: " + desc,
                                "high", base + ep, "", "", resp.body.substr(0, 100), ""
                            });
                        }
                    }
                }
        ...
        {"WP Plugin Vulnerability Check", [](const Config &cfg, HttpClient &http, const CrawlResult &crawl) -> std::vector<Finding> {
            std::vector<Finding> findings;
            ...
            for (const auto &[path, desc] : plugin_checks) {
                ...
                if (resp.status_code == 200 && resp.body.size() > 50) {
                    ...
                    if (path.find("debug.log") != std::string::npos ||
                        path.find("config") != std::string::npos ||
                        path.find("backups") != std::string::npos)
                        sev = "high";
                    findings.push_back({
                        "WP Sensitive File: " + desc, sev,
                        base + path, "", "", resp.body.substr(0, 100), ""
                    });
                }
            }
            return findings;
        ...
        {"WP SSRF via Webhooks", [](const Config &cfg, HttpClient &http, const CrawlResult &crawl) -> std::vector<Finding> {
            std::vector<Finding> findings;
            std::string base = crawl.urls.empty() ? "https://" + cfg.target : crawl.urls[0].substr(0, crawl.urls[0].find("/", 8));
            ...
            for (const auto &ep : ssrf_endpoints) {
                ...
                if (resp.status_code == 200 && (
                    ...
                    resp.body.find("instance-id") != std::string::npos)) {
                    findings.push_back({
                        "SSRF via WordPress endpoint", "critical",
                        base + ep, "", "", resp.body.substr(0, 100), ""
                    });
                }
            }
            return findings;
