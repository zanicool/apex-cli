...
std::vector<Finding> scan_cms_admin_exposure(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& [path, cms] : panels) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 200 && resp.body.find("404") == std::string::npos) {
      findings.push_back({"CMS Admin Exposed", "high", base + path, cms + " admin panel accessible from internet", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_cms_debug_mode(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& [path, desc] : debug_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 100) {
      findings.push_back({"Debug Mode Exposed", "high", base + path, desc + " accessible in production", "", "", ""});
    }
  }
  ...
  const std::vector<std::string> debug_sigs = {
      "Traceback (most recent", "DJANGO_SETTINGS_MODULE", "joomla-debug", "system-debug", "Xdebug", "APP_DEBUG=true", "WP_DEBUG"};
  for (const auto& sig : debug_sigs) {
    if (home.body.find(sig) != std::string::npos) {
      findings.push_back({"Debug Mode Active", "high", base, "Debug indicator found: " + sig, "", "", ""});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_graphql_introspection(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> gql_paths = {"/graphql", "/graphql/", "/api/graphql", "/v1/graphql", "/gql", "/query", "/graphiql"};
  const std::string introspection = R"({"query":"{ __schema { types { name } } }"})";
  ...
  for (const auto& path : gql_paths) {
    ...
    if (resp.status_code == 200 && resp.body.find("__schema") != std::string::npos) {
      findings.push_back(
          {"GraphQL Introspection", "medium", base + path, "GraphQL introspection enabled — full schema exposed", "", "", ""});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_default_creds(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  const DefaultCred creds[] = {
      {"/phpmyadmin/index.php", "pma_username=root&pma_password=", "application/x-www-form-urlencoded", "server_databases",
       "phpMyAdmin (root/empty)"},
      {"/api/login", R"({"username":"admin","password":"admin"})", "application/json", "token", "API (admin/admin)"},
      {"/api/login", R"({"username":"admin","password":"password"})", "application/json", "token", "API (admin/password)"},
  };

  for (const auto& c : creds) {
    auto resp = http.post(base + c.path, c.post_data, c.content_type);
    if (resp.status_code == 200 && resp.body.find(c.success_indicator) != std::string::npos) {
      findings.push_back({"Default Credentials", "critical", base + c.path, std::string(c.platform) + " — default creds work", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_wp_misconfig(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  auto xmlrpc = http.post(base + "/xmlrpc.php",
                          "<?xml version=\"1.0\"?><methodCall><methodName>system.listMethods</methodName></methodCall>", "text/xml");
  if (xmlrpc.body.find("wp.getUsersBlogs") != std::string::npos)
    findings.push_back({"WP xmlrpc.php", "medium", base + "/xmlrpc.php", "xmlrpc.php enabled — brute-force & DDoS vector", "", "", ""});

  // User enumeration
  auto users = http.get(base + "/wp-json/wp/v2/users");
  if (users.status_code == 200 && users.body.find("\"slug\"") != std::string::npos)
    findings.push_back({"WP User Enum", "medium", base + "/wp-json/wp/v2/users", "User enumeration via REST API", "", "", ""});

  // wp-config.php backup
  const std::vector<std::string> config_paths = {"/wp-config.php.bak", "/wp-config.php~", "/wp-config.php.old", "/wp-config.php.swp",
                                                 "/wp-config.txt"};
  for (const auto& p : config_paths) {
    auto resp = http.get(base + p);
    if (resp.status_code == 200 && resp.body.find("DB_PASSWORD") != std::string::npos) {
      findings.push_back({"WP Config Leak", "critical", base + p, "wp-config backup with DB credentials exposed", "", "", ""});
      break;
    }
  }
  ...
  if (debug_log.status_code == 200 && debug_log.body.size() > 50)
    findings.push_back({"WP Debug Log", "medium", base + "/wp-content/debug.log", "WordPress debug.log publicly accessible", "", "", ""});

  // Uploads directory listing
  auto uploads = http.get(base + "/wp-content/uploads/");
  if (uploads.body.find("Index of") != std::string::npos)
    findings.push_back({"WP Uploads Listing", "medium", base + "/wp-content/uploads/", "Directory listing on uploads folder", "", "", ""});

  return findings;
}

/// Joomla/Drupal/Magento specific misconfigs.
std::vector<Finding> scan_platform_misconfig(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  auto home = http.get(base + "/");
  ...
  // Joomla
  if (home.body.find("Joomla") != std::string::npos || home.body.find("/administrator/") != std::string::npos) {
    auto cfg = http.get(base + "/configuration.php.bak");
    if (cfg.status_code == 200 && cfg.body.find("$host") != std::string::npos)
      findings.push_back(
          {"Joomla Config Leak", "critical", base + "/configuration.php.bak", "Joomla config backup with DB creds", "", "", ""});
  }
  ...
  if (home.body.find("Drupal") != std::string::npos) {
    ...
    if (changelog.status_code == 200 && changelog.body.find("Drupal") != std::string::npos)
      findings.push_back({"Drupal Version Leak", "low", base + "/CHANGELOG.txt", "Drupal CHANGELOG.txt exposes exact version", "", "", ""});
  }
  ...
  if (home.body.find("Mage") != std::string::npos) {
    ...
    if (downloader.status_code == 200 && downloader.body.find("Magento") != std::string::npos)
      findings.push_back({"Magento Downloader", "high", base + "/downloader/", "Magento Connect Manager exposed", "", "", ""});
  }
  ...
  if (crx.status_code == 200 && crx.body.find("Package Manager") != std::string::npos)
    findings.push_back(
        {"AEM Package Manager", "critical", base + "/crx/packmgr/index.jsp", "AEM CRX Package Manager exposed — RCE possible", "", "", ""});
  ...
  if (aem_users.status_code == 200 && aem_users.body.find("rep:User") != std::string::npos)
    findings.push_back({"AEM User Enum", "high", base, "AEM QueryBuilder exposes user list", "", "", ""});
