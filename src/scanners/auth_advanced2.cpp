...
std::vector<Finding> scan_weak_password_policy(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> reg_paths = {"/api/register", "/api/signup", "/register", "/api/auth/register"};
...
std::vector<Finding> scan_account_enum_register(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> reg_paths = {"/api/register", "/api/signup", "/register"};

  for (const auto& path : reg_paths) {
    // Try registering with likely-existing email
    auto resp1 = http.post(
        base + path,
        R"({"email":"admin@)" +
            std::string(crawl.urls[0].substr(crawl.urls[0].find("://") + 3, crawl.urls[0].find("/", 8) - crawl.urls[0].find("://") - 3)) +
            R"(","password":"Test12345!"})",
        "application/json");
    // Try with definitely-not-existing email
    auto resp2 =
        http.post(base + path, R"({"email":"definitely_not_exists_xyz@nonexistent.invalid","password":"Test12345!"})", "application/json");

    if (resp1.status_code != resp2.status_code ||
        (resp1.body.find("exists") != std::string::npos && resp2.body.find("exists") == std::string::npos) ||
        (resp1.body.find("taken") != std::string::npos && resp2.body.find("taken") == std::string::npos)) {
      findings.push_back({"Account Enumeration (Registration)", "low", base + path, "Different responses reveal whether email exists",
                          "email", "", "Existing vs non-existing email gives different response"});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_account_enum_forgot(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> paths = {"/api/forgot-password", "/api/password-reset", "/forgot-password"};

  for (const auto& path : paths) {
    auto resp1 = http.post(base + path, R"({"email":"admin@example.com"})", "application/json");
    auto resp2 = http.post(base + path, R"({"email":"nonexist_xyz_abc@nowhere.invalid"})", "application/json");

    if (resp1.body != resp2.body && resp1.status_code == resp2.status_code) {
      findings.push_back({"Account Enumeration (Forgot Password)", "low", base + path,
                          "Different response for existing vs non-existing email", "email", "", "Responses differ in content"});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_token_in_url(const Config&, HttpClient&, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (std::regex_search(url, m, token_re)) {
      findings.push_back({"Sensitive Token in URL", "medium", url, "Auth token in query string — visible in logs/referrer", m[1].str(),
                          m[2].str().substr(0, 15) + "...", "Token exposed in URL"});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_no_session_expiry(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  for (const auto& [h, v] : resp.headers) {
    ...
    if (v.find("session") != std::string::npos || v.find("token") != std::string::npos) {
      if (v.find("Max-Age=") == std::string::npos && v.find("Expires=") == std::string::npos) {
        findings.push_back({"No Session Expiry", "low", crawl.urls[0], "Session cookie has no expiry — persists until browser closes", "",
                            "", v.substr(0, 80)});
      } else if (v.find("Max-Age=31536000") != std::string::npos || v.find("Max-Age=63072000") != std::string::npos) {
        findings.push_back(
            {"Excessive Session Duration", "medium", crawl.urls[0], "Session cookie expires in 1+ years", "", "", v.substr(0, 80)});
      }
      break;
    }
  }
...
std::vector<Finding> scan_oauth_redirect_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> oauth_paths = {"/oauth/authorize", "/auth/authorize", "/connect/authorize", "/api/oauth/authorize"};
  ...
  for (const auto& path : oauth_paths) {
    for (const auto& uri : bypass_uris) {
      auto resp = http.get(base + path + "?redirect_uri=" + uri + "&response_type=code&client_id=test");
      auto loc = resp.headers.find("Location");
      if (loc != resp.headers.end() && loc->second.find("evil.com") != std::string::npos) {
        findings.push_back({"OAuth Redirect URI Bypass", "high", base + path, "redirect_uri validation bypassed", "redirect_uri", uri,
...
std::vector<Finding> scan_registration_role(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> reg_paths = {"/api/register", "/api/signup", "/api/auth/register"};
...
std::vector<Finding> scan_concurrent_sessions(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& path : login_paths) {
    auto r1 = http.post(base + path, R"({"email":"test@test.com","password":"test"})", "application/json");
    auto r2 = http.post(base + path, R"({"email":"test@test.com","password":"test"})", "application/json");
    if (r1.status_code == 200 && r2.status_code == 200 && r1.body.find("token") != std::string::npos &&
      ...
      if (r1.body != r2.body) {
        findings.push_back({"No Concurrent Session Limit", "low", base + path, "Multiple active sessions allowed simultaneously", "", "",
...
std::vector<Finding> scan_invite_link(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
...
std::vector<Finding> scan_reset_token_leak(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> reset_paths = {"/api/forgot-password", "/api/password-reset", "/api/auth/reset"};
    ...
    if (resp.status_code == 200) {
      ...
      if (std::regex_search(resp.body, m, token_re)) {
        findings.push_back({"Password Reset Token Leaked", "critical", base + path,
                            "Reset token returned in API response instead of email-only", "", m[1].str().substr(0, 15) + "...",
