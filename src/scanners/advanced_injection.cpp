...
/// LDAP Injection.
std::vector<Finding> scan_ldap_injection(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  const std::vector<std::string> ldap_payloads = {
      "*)(uid=*))(|(uid=*", "*)(&", "*)(|(&"};
    ...
    for (const auto &param : ldap_params) {
      ...
      for (const auto &payload : ldap_payloads) {
        ...
        if (resp.status_code == 200 && resp.body != baseline.body &&
            ...
            (resp.body.find("uid") != std::string::npos ||
             resp.body.find("cn=") != std::string::npos ||
             resp.body.find("dn:") != std::string::npos)) {
          findings.push_back({"LDAP Injection", "critical", url,
                              "LDAP query manipulated via " + param,
                              param, payload,
...
/// XPath Injection.
std::vector<Finding> scan_xpath_injection(const Config &, HttpClient &http,
                                           const CrawlResult &crawl) {
  std::vector<Finding> findings;
  const std::vector<std::string> xpath_payloads = {
      "' or '1'='1", "'] | //* | //*['", "1 or 1=1"};
  ...
  for (const auto &url : crawl.urls) {
    for (const auto &p : crawl.params) {
      ...
      for (const auto &payload : xpath_payloads) {
        ...
        if (resp.status_code == 200 && resp.body.size() > baseline.body.size() * 2 &&
            resp.body.size() > 100) {
          findings.push_back({"XPath Injection", "high", url,
                              "XPath query manipulated — excessive data returned",
                              p.name, payload,
                              "Response " + std::to_string(resp.body.size()) +
                                  " vs baseline " + std::to_string(baseline.body.size())});
          goto next_xpath;
        }
      }
    }
...
std::vector<Finding> scan_log_injection(const Config &, HttpClient &http,
  ...
  for (const auto &url : crawl.urls) {
    auto resp = http.get(url, {
        ...
        {"User-Agent", log_payload}});
    // We can't easily verify log injection, but if server reflects the header...
    if (resp.body.find("CRITICAL") != std::string::npos ||
        resp.body.find("Fake log") != std::string::npos) {
      findings.push_back({"Log Injection", "medium", url,
                          "Injected content reflected — potential log forging",
                          "User-Agent/X-Forwarded-For", log_payload,
                          "Injected text found in response"});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_header_cmdi(const Config &, HttpClient &http,
                                       const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  for (const auto &url : crawl.urls) {
    for (const auto &[header, payload] : header_payloads) {
      ...
      if (resp.body.find(canary) != std::string::npos) {
        findings.push_back({"Command Injection via Header", "critical", url,
                            "OS command executed via " + header + " header",
                            header, payload,
                            "Canary '" + canary + "' found in response"});
        return findings;
      }
    }
  }
...
std::vector<Finding> scan_deserialization(const Config &, HttpClient &http,
                                           const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  const std::vector<std::string> deser_paths = {
      "/api/import", "/api/data", "/api/restore", "/deserialize",
  ...
  for (const auto &path : deser_paths) {
    ...
    if (resp.status_code == 500 &&
        (resp.body.find("ClassNotFoundException") != std::string::npos ||
         resp.body.find("InvalidClassException") != std::string::npos ||
         resp.body.find("java.io") != std::string::npos)) {
      findings.push_back({"Insecure Deserialization", "critical", base + path,
                          "Server attempts to deserialize Java objects",
  ...
  for (const auto &path : deser_paths) {
    ...
    if (resp.status_code == 500 &&
        (resp.body.find("unserialize") != std::string::npos ||
         resp.body.find("__wakeup") != std::string::npos)) {
      findings.push_back({"PHP Deserialization", "critical", base + path,
                          "Server processes PHP serialized objects",
                          "", php_payload,
                          "Error: " + resp.body.substr(0, 150)});
      break;
    }
  ...
  for (const auto &url : crawl.urls) {
    for (const auto &p : crawl.params) {
      ...
      if (resp.body.find("apex_ssi_test") != std::string::npos) {
        findings.push_back({"SSI Injection", "critical", url,
                            "Server-Side Include command executed",
                            p.name, ssi_payload,
...
std::vector<Finding> scan_xxe_content_type(const Config &, HttpClient &http,
                                            const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> api_paths = {
      "/api/login", "/api/user", "/api/data", "/api/search", "/api/import"};
  ...
  for (const auto &path : api_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 0 &&
      ...
      if (resp.body.find("<?xml") == std::string::npos &&
          resp.body.size() < 200 && resp.body.size() > 1) {
        findings.push_back({"XXE via Content-Type Switch", "critical", base + path,
                            "XML parsed when switching Content-Type from JSON to XML",
                            "", "file:///etc/hostname",
...
std::vector<Finding> scan_integer_overflow(const Config &, HttpClient &http,
                                            const CrawlResult &crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto &url : crawl.urls) {
    for (const auto &p : crawl.params) {
      ...
      if (resp.status_code == 500 ||
          resp.body.find("overflow") != std::string::npos ||
          resp.body.find("integer") != std::string::npos) {
        findings.push_back({"Integer Overflow", "medium", url,
                            "Server crashes or errors with large integer",
                            p.name, large_num,
      ...
      if (neg_resp.status_code == 500) {
        findings.push_back({"Negative Integer Handling", "low", url,
                            "Server errors with negative value",
                            p.name, negative,
