...
std::vector<Finding> scan_auth_surface_map(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  std::vector<std::pair<std::string, std::string>> paths = {
      ...
      {"/api/auth/confirm", "verify"},
  };

  for (const auto& [path, type] : paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 405 || resp.status_code == 400 || resp.status_code == 422 || resp.status_code == 200) {
      if (resp.body.find("Access Denied") == std::string::npos && resp.body.find("<!DOCTYPE html>") == std::string::npos) {
      ...
      if (reg.status_code == 200 || reg.status_code == 201) {
        if (reg.body.find("admin") != std::string::npos || reg.body.find("staff") != std::string::npos) {
          findings.push_back(Finding{"Registration Mass Assignment", "critical", base + ep.path,
                                     ...
                                     "role", "admin", reg.body.substr(0, 200)});
        }
      }
    }
  }
  ...
  for (const auto& ep : endpoints) {
    if (ep.type != "register") continue;
    auto existing = http.post(base + ep.path, R"({"email":"admin@)" + base.substr(base.find("://") + 3) + R"(","password":"test123"})",
                              "application/json");
    if (existing.body.find("already exists") != std::string::npos || existing.body.find("already registered") != std::string::npos ||
      ...
      findings.push_back(Finding{"User Enumeration via Registration", "medium", base + ep.path,
                                 ...
                                 "email", "admin@...", "Response differs for existing vs non-existing accounts"});
    }
    break;
  }
  ...
  for (const auto& ep : endpoints) {
    ...
    auto real = http.post(base + ep.path, R"({"email":"admin@)" + base.substr(base.find("://") + 3) + R"("})", "application/json");
    auto fake = http.post(base + ep.path, R"({"email":"nonexistent_apex_test_12345@nowhere.invalid"})", "application/json");
    if (real.body != fake.body || real.status_code != fake.status_code) {
      findings.push_back(Finding{"User Enumeration via Password Reset", "medium", base + ep.path,
                                 "Password reset responds differently for existing vs non-existing emails.", "email", "",
                                 "Existing: " + std::to_string(real.status_code) + ", Non-existing: " + std::to_string(fake.status_code)});
    }
    break;
  }
...
std::vector<Finding> scan_account_takeover(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& path : reset_paths) {
    ...
    if (resp.status_code == 200 &&
        (resp.body.find("token") != std::string::npos || resp.body.find("reset_url") != std::string::npos ||
      ...
      findings.push_back(Finding{"Password Reset Token in Response", "critical", base + path,
                                 ...
                                 "", path, resp.body.substr(0, 200)});
      return findings;
    }
  }

  // Check if password change doesn't require current password
  std::vector<std::string> change_paths = {"/api/auth/change-password", "/api/v1/auth/change-password", "/api/password/change",
                                           "/api/user/password"};
...
std::vector<Finding> scan_2fa_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  std::vector<std::string> verify_paths = {"/api/auth/2fa/verify", "/api/v1/auth/verify-otp", "/api/auth/verify", "/api/mfa/verify"};
