...
std::vector<Finding> scan_jwt(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  if (dot1 != std::string::npos) {
    ...
    if (test.status_code == 200) {
      findings.push_back({"JWT None Alg", "critical", crawl.urls[0], "JWT accepts 'none' algorithm", "", none_token, ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_2fa_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& path : post_2fa) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 200 && resp.body.find("verify") == std::string::npos &&
...
std::vector<Finding> scan_session_fixation(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  findings.push_back({"Session Fixation", "medium", crawl.urls[0], "Session ID not regenerated", "", "", session1});
  return findings;
}
...
std::vector<Finding> scan_timing_oracle(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& path : login_paths) {
    ...
    auto dur1 = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start1).count();

    auto start2 = std::chrono::steady_clock::now();
    http.post(base + path, "username=nonexistent_xyz&password=wrong", "application/x-www-form-urlencoded");
    auto dur2 = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start2).count();

    long diff = std::abs(dur1 - dur2);
    if (diff > 100) {
      findings.push_back({"Timing Oracle", "medium", base + path, "Username enumeration via timing (" + std::to_string(diff) + "ms diff)",
                          "username", "", ""});
    }
  }
  return findings;
...
std::vector<Finding> scan_token_race(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& path : token_paths) {
    auto r1 = http.post(base + path, "{\"email\":\"test@test.com\"}", "application/json");
    auto r2 = http.post(base + path, "{\"email\":\"test@test.com\"}", "application/json");
    if (r1.status_code == 200 && r2.status_code == 200 && r1.body == r2.body && !r1.body.empty()) {
      findings.push_back({"Token Race", "medium", base + path, "Same token issued for concurrent requests", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_login_security(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& url : login_urls) {
    ...
    // Check for CAPTCHA.
    bool has_captcha = resp.body.find("captcha") != std::string::npos || resp.body.find("recaptcha") != std::string::npos ||
                       resp.body.find("hcaptcha") != std::string::npos || resp.body.find("turnstile") != std::string::npos;
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      if (r.status_code == 429 || r.status_code == 403 || r.body.find("locked") != std::string::npos ||
          r.body.find("blocked") != std::string::npos || r.body.find("too many") != std::string::npos) {
        has_lockout = true;
        break;
      }
    }
