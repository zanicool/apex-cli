...
std::vector<Finding> scan_waf_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  std::vector<Mutation> mutations = {
      {"Double Encode", double_encode(base_payload)},   {"Unicode Escape", unicode_escape(base_payload)},
      {"Comment Inject", comment_inject("' OR 1=1--")}, {"Case Randomize", "<ScRiPt>alert(1)</sCrIpT>"},
      {"Null Byte", "<scr%00ipt>alert(1)</script>"},    {"Tab/Newline", "<scr\tipt>alert(1)</script>"},
  };
    ...
    for (const auto& [base, param] : targets) {
      ...
      for (const auto& mut : mutations) {
        ...
        if (resp.status_code == 200) {
          findings.push_back({"WAF Bypass", "high", url, "WAF bypassed via " + mut.name, param, mut.payload, ""});
          break;
        }
      }
    }
...
std::vector<Finding> scan_js_secrets(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& url : crawl.urls) {
    if (url.find(".js") == std::string::npos && url.find("javascript") == std::string::npos) continue;
    auto resp = http.get(url);
    if (resp.status_code != 200) continue;
    for (const auto& [re, desc] : patterns) {
      if (std::regex_search(resp.body, re)) {
        findings.push_back({"JS Secret", "high", url, "Potential " + desc + " in JS", "", "", ""});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_source_maps(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (resp.body.find("sourceMappingURL") != std::string::npos) {
      ...
      if (map_resp.status_code == 200 && map_resp.body.find("\"sources\"") != std::string::npos) {
        findings.push_back({"Source Map", "medium", map_url, "JavaScript source map exposed", "", "", ""});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_dependency_confusion(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (; it != end; ++it) {
    ...
    if (npm.status_code == 404) {
      findings.push_back({"Dependency Confusion", "high", base + "/package.json", "Internal package not on npm: " + full, "", full, ""});
    }
  }
  return findings;
}
