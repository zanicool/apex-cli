...
std::vector<Finding> scan_whatsapp_intel(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (; it != std::sregex_iterator(); ++it) phones.insert((*it)[0].str());

  // Also check contact/about pages.
  const std::vector<std::string> contact_paths = {"/contact",    "/about",    "/impressum", "/kontakt", "/contacto",
  ...
  for (const auto& path : contact_paths) {
    ...
    for (; pit != std::sregex_iterator(); ++pit) phones.insert((*pit)[0].str());
  }
  ...
  for (const auto& phone : phones) {
    ...
    // Strip + and spaces.
    clean.erase(std::remove_if(clean.begin(), clean.end(), [](char c) { return !isdigit(c); }), clean.end());

    // wa.me redirect check — confirms WhatsApp registration.
    std::string wa_url = "https://wa.me/" + clean;
    auto resp = http.get(wa_url);
    if (resp.status_code == 200 && resp.body.find("api.whatsapp.com") != std::string::npos) {
      findings.push_back({"WhatsApp Confirmed", "info", wa_url, "Phone " + phone + " is registered on WhatsApp", "", "", ""});
    }
    ...
    if (biz.status_code == 200 && biz.body.size() > 500) {
      ...
      if (biz.body.find("address") != std::string::npos) detail += " — address exposed";
      if (biz.body.find("email") != std::string::npos) detail += " — email exposed";
      if (biz.body.find("website") != std::string::npos) detail += " — website linked";
      findings.push_back({"WhatsApp Business Profile", "medium", biz_url, detail, "", phone, ""});
    }
  }
  ...
  for (auto gi = grp_begin; gi != std::sregex_iterator(); ++gi) {
    ...
    if (gresp.status_code == 200) {
      ...
      if (pos != std::string::npos) {
        ...
        if (end != std::string::npos) group_name = gresp.body.substr(pos, end - pos);
      }
      findings.push_back({"WhatsApp Group Leak", "high", link, "Public WhatsApp group invite: " + group_name, "", "", ""});
    }
  }
...
std::vector<Finding> scan_ooo_probing(const Config& cfg, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> pages = {"/", "/contact", "/about", "/team", "/impressum", "/support"};
  ...
  const std::vector<std::string> prefixes = {"info", "contact", "support", "admin", "hr", "sales", "office", "hello", "team", "security"};
  for (const auto& p : prefixes) emails.insert(p + "@" + domain);
  ...
  if (autodiscover.status_code == 200 || autodiscover.status_code == 401) {
    findings.push_back({"Autodiscover Exposed", "medium", "https://autodiscover." + domain,
  ...
  if (owa.status_code == 200 || owa.status_code == 302) {
    findings.push_back({"OWA Exposed", "medium", "https://mail." + domain + "/owa/",
  ...
  if (ews.status_code == 200 || ews.status_code == 401) {
    findings.push_back({"EWS Exposed", "high", "https://mail." + domain + "/EWS/Exchange.asmx",
  ...
  auto tenant = http.get("https://login.microsoftonline.com/" + domain + "/.well-known/openid-configuration");
  if (tenant.status_code == 200 && tenant.body.find("token_endpoint") != std::string::npos) {
    findings.push_back({"M365 Tenant Confirmed", "info", "https://login.microsoftonline.com/" + domain,
  ...
  for (const auto& form : crawl.forms) {
    ...
    for (const auto& f : form.fields) {
      if (f.name.find("email") != std::string::npos || f.name.find("mail") != std::string::npos) {
...
std::vector<Finding> scan_contact_harvest(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Check for vCard/contact exports.
  const std::vector<std::string> vcard_paths = {"/contact.vcf", "/team.vcf", "/contacts.vcf", "/staff.vcf", "/.well-known/carddav"};
  for (const auto& path : vcard_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && (resp.body.find("BEGIN:VCARD") != std::string::npos || resp.body.find("FN:") != std::string::npos)) {
      findings.push_back({"vCard Exposed", "medium", base + path, "vCard file with contact details publicly accessible", "", "", ""});
    }
  }

  // Check for staff/team directory pages leaking too much.
  const std::vector<std::string> dir_paths = {"/team", "/staff", "/people", "/directory", "/our-team", "/about/team", "/employees"};
  for (const auto& path : dir_paths) {
    ...
    if (signals >= 2) {
      findings.push_back(
          {"Staff Directory Exposed", "medium", base + path, "Team page leaks emails + phone numbers + social profiles", "", "", ""});
    }
  }
