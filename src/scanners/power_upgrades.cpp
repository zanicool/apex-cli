...
std::vector<Finding> scan_js_endpoints(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& url : crawl.urls) {
    if (url.find(".js") != std::string::npos || url.find("/static/js/") != std::string::npos || url.find("/_next/") != std::string::npos)
      js_urls.insert(url);
  }
  // Iterate over targets.
  for (const auto& p : {"/main.js", "/app.js", "/bundle.js", "/vendor.js", "/_next/static/chunks/main.js", "/static/js/main.js"})
  ...
  for (const auto& js_url : js_urls) {
    ...
    for (const auto& pat : patterns) {
      ...
      for (auto it = begin; it != end; ++it) {
        ...
        if (ep.size() >= 3 && ep.size() <= 200 && ep.find(".css") == std::string::npos && ep.find(".png") == std::string::npos)
  ...
  for (const auto& ep : discovered) {
    ...
    if (resp.status_code != 404 && resp.status_code < 500) live.push_back(full);
    if (live.size() >= 20) break;
  }

  if (!live.empty()) {
    std::string detail = "Found " + std::to_string(live.size()) + " live API endpoints in JavaScript";
    findings.push_back({"JS Endpoint Discovery", "info",
                        std::to_string(live.size()) + " endpoints from " + std::to_string(js_urls.size()) + " JS files", detail, "", "",
                        ""});
  }
...
std::vector<Finding> scan_authenticated(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  // Early return if no URLs to scan.
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> reg_paths = {"/register", "/signup", "/api/register", "/api/auth/register", "/api/v1/register"};
  ...
  for (const auto& path : reg_paths) {
    std::string body = R"({"email":")" + test_email + R"(","password":"ApexTest123!","username":"apextest"})";
  ...
  findings.push_back({"Auto-Registration Successful", "info", base,
                      "Registered and obtained auth token (len=" + std::to_string(auth_token.size()) + ")", "", "", ""});

  // Probe authenticated endpoints.
  const std::vector<std::string> auth_paths = {"/api/me",    "/api/user",  "/api/profile",  "/api/account",
  ...
  for (const auto& path : auth_paths) {
    ...
    if (authed.status_code == 200 && (unauth.status_code == 401 || unauth.status_code == 403)) {
      ...
      for (const auto& id_path : {path + "/1", path + "/2", path + "?user_id=1"}) {
        ...
        if (id_resp.status_code == 200 && id_resp.body != authed.body && id_resp.body.size() > 50) {
...
std::vector<Finding> scan_param_bruteforce(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (size_t i = 0; i < limit; ++i) {
    ...
    for (size_t j = 0; j < params.size(); j += 10) {
      ...
      for (size_t k = j; k < end; ++k) param_str += params[k] + "=apex_test&";
      ...
      for (size_t k = j; k < end; ++k) {
        ...
        if (indiv.status_code == 200) {
          ...
          if (std::abs(idiff) > 20) {
            findings.push_back({"Hidden Parameter: " + params[k], "medium", crawl.urls[i] + sep + params[k] + "=apex_test",
                                "Param '" + params[k] + "' changes response (size diff: " + std::to_string(idiff) + " bytes)", params[k],
                                "", ""});
          }
        }
      }
...
std::vector<Finding> scan_differential(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (size_t i = 0; i < limit; ++i) {
    ...
    for (const auto& ah : auth_headers) {
      ...
      if (resp.status_code == 200 && resp.body != base_resp.body) {
        ...
        if (diff > 50) {
          findings.push_back({"Differential: Extra Data with " + std::string(ah.name), "high", crawl.urls[i],
                              std::string("Header '") + ah.header + ": " + ah.value + "' reveals " + std::to_string(diff) + " extra bytes",
                              "", "", ""});
        }
      }
    }
    ...
    for (const auto& method : {"POST", "PUT", "PATCH", "DELETE"}) {
      ...
      if (resp.status_code == 200 && resp.body != base_resp.body && (int)resp.body.size() > (int)base_resp.body.size() + 50) {
        findings.push_back(
            {"Differential: " + std::string(method) + " Returns Extra Data", "medium", crawl.urls[i],
             std::string(method) + " returns " + std::to_string(resp.body.size() - base_resp.body.size()) + " more bytes than GET", "", "",
             ""});
        break;
      }
    }
    ...
    for (const auto& ct : {"application/json", "application/xml", "text/xml"}) {
      ...
      if (resp.status_code == 200 && resp.body != base_resp.body &&
          (resp.body.find("stack") != std::string::npos || resp.body.find("trace") != std::string::npos ||
           resp.body.find("debug") != std::string::npos)) {
        findings.push_back({"Content-Type Confusion — Debug Info Leak", "medium", crawl.urls[i],
