...
std::vector<Finding> scan_github_leaks(const Config &cfg, HttpClient &http,
                                       const CrawlResult &) {
  ...
  for (const auto &[query, desc] : queries) {
    ...
    if (resp.status_code == 200 && resp.body.find("code-list") != std::string::npos &&
        resp.body.find("We couldn") == std::string::npos) {
      findings.push_back({"GitHub Secret Leak", "critical", url,
                          desc + " — found on GitHub", "", query, ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_paste_leaks(const Config &cfg, HttpClient &http,
                                      const CrawlResult &) {
  std::vector<Finding> findings;
  std::string domain = get_domain(cfg);
  ...
  for (const auto &dork : paste_dorks) {
    ...
    auto resp = http.get(url);
    if (resp.status_code == 200 && resp.body.find("did not match") == std::string::npos &&
        resp.body.find("result") != std::string::npos &&
        resp.body.find(domain) != std::string::npos) {
      findings.push_back({"Paste Site Leak", "high", url,
                          "Domain mentioned on paste site — possible credential dump",
                          "", dork, ""});
    }
  }
  return findings;
...
std::vector<Finding> scan_credential_exposure(const Config &cfg, HttpClient &http,
  ...
  // Add common patterns.
  for (const auto &p : {"info", "admin", "contact", "support", "hr", "security"})
    emails.insert(std::string(p) + "@" + domain);
  ...
  for (const auto &email : emails) {
    ...
    if (resp.status_code == 200 && resp.body.find("Name") != std::string::npos) {
      ...
      findings.push_back({"Credential Breach", "high",
                          "https://haveibeenpwned.com/account/" + email,
                          email + " found in " + std::to_string(count) + " breach(es)",
                          "", email, ""});
    }
  }
  ...
  if (domain_resp.status_code == 200 && domain_resp.body.size() > 10 &&
      domain_resp.body.find("Name") != std::string::npos) {
    findings.push_back({"Domain Breach History", "high",
                        "https://haveibeenpwned.com",
                        "Domain " + domain + " appears in known data breaches",
                        "", domain, ""});
  }
...
std::vector<Finding> scan_api_key_exposure(const Config &cfg, HttpClient &http,
                                           const CrawlResult &crawl) {
  std::vector<Finding> findings;
  ...
  for (; jit != std::sregex_iterator(); ++jit) {
    ...
    if (js_path.find("http") == 0) js_urls.insert(js_path);
    else js_urls.insert(base + (js_path[0] == '/' ? "" : "/") + js_path);
  }
  ...
  for (const auto &js_url : js_urls) {
    ...
    for (const auto &pat : patterns) {
      ...
      if (std::regex_search(resp.body, re)) {
        findings.push_back({"Exposed " + std::string(pat.name), pat.severity, js_url,
                            std::string(pat.name) + " found in JavaScript file",
                            "", "", ""});
  ...
  for (const auto &pat : patterns) {
    ...
    if (std::regex_search(home.body, re)) {
      findings.push_back({"Exposed " + std::string(pat.name), pat.severity, base,
                          std::string(pat.name) + " found in page source",
                          "", "", ""});
    }
  }
...
std::vector<Finding> scan_brand_exposure(const Config &cfg, HttpClient &http,
                                         const CrawlResult &) {
  std::vector<Finding> findings;
  std::string domain = get_domain(cfg);
  ...
  if (ct.status_code == 200 && ct.body.size() > 100) {
    ...
    for (; dit != std::sregex_iterator(); ++dit) {
      ...
      if (found.find(domain) == std::string::npos && found.find(name) != std::string::npos)
        suspicious.insert(found);
    }
    if (!suspicious.empty()) {
      std::string detail = std::to_string(suspicious.size()) + " lookalike domain(s) found: ";
      int shown = 0;
      for (const auto &s : suspicious) {
        if (++shown > 5) { detail += "..."; break; }
        detail += s + ", ";
      }
      findings.push_back({"Typosquatting/Phishing Domains", "high",
                          "https://crt.sh/?q=%25" + name + "%25",
                          detail, "", "", ""});
    }
  }
  ...
  if (ransom.status_code == 200 && ransom.body.find(domain) != std::string::npos &&
    ...
    findings.push_back({"Ransomware Mention", "critical",
                        "https://www.google.com/search?q=" + ransom_dork,
...
std::vector<Finding> scan_infostealer_indicators(const Config &cfg, HttpClient &http,
                                                  const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto &[name, value] : resp.headers) {
    ...
    bool session_cookie = (value.find("session") != std::string::npos ||
                           value.find("PHPSESSID") != std::string::npos ||
                           value.find("JSESSIONID") != std::string::npos ||
                           value.find("connect.sid") != std::string::npos ||
    ...
    if (value.find("__Host-") == std::string::npos &&
        value.find("__Secure-") == std::string::npos)
      issues += "no cookie prefix, ";
    ...
    if (!issues.empty()) {
      findings.push_back({"Session Theft Risk", "high", base,
                          "Session cookie vulnerable to infostealer theft: " + issues,
                          "", "", ""});
    }
  }
  ...
  for (const auto &path : login_paths) {
    ...
    if (login_resp.body.find("device") == std::string::npos &&
        login_resp.body.find("fingerprint") == std::string::npos &&
        login_resp.body.find("mfa") == std::string::npos &&
        login_resp.body.find("2fa") == std::string::npos) {
...
std::vector<Scanner> register_leak_intel_scanners() {
  return {
      {"GitHub Secret Scan", scan_github_leaks},
      {"Paste Site Leaks", scan_paste_leaks},
      {"Credential Exposure", scan_credential_exposure},
      {"API Key Exposure", scan_api_key_exposure},
      {"Brand/Phishing Monitor", scan_brand_exposure},
      {"Infostealer Risk", scan_infostealer_indicators},
  };
}
