      ...
      for (const auto &p : payloads) {
        ...
        if (resp.status_code == 200 && resp.body.find(p.expected) != std::string::npos) {
          ...
          if (check.body.find(p.expected) == std::string::npos) {
            findings.push_back(Finding{"SSTI — " + p.engine + " CONFIRMED", "critical",
                                base_url + p.payload,
                                "Server-Side Template Injection confirmed via " + p.engine + ". "
                                "Payload '" + p.payload + "' evaluated to '" + p.expected + "'. "
...
std::vector<Finding> scan_crlf_injection(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
...
std::vector<Finding> scan_deserialization(const Config &, HttpClient &http,
  ...
  std::vector<std::string> php_paths = {
      "/api/import", "/api/upload", "/api/data"};
  for (const auto &path : php_paths) {
    auto r = http.get(base + path);
    if (r.status_code == 200 || r.status_code == 400) {
      if (r.body.find("unserialize") != std::string::npos ||
          r.body.find("O:") != std::string::npos) {
...
std::vector<Finding> scan_open_redirect(const Config &, HttpClient &http,
  ...
  std::regex redirect_param_re(R"x([?&](next|url|redirect|return|goto|continue|dest|destination|redir|returnUrl|return_to|forward|target|out|view|ref|r)=)x");
    ...
    if (std::regex_search(url, m, redirect_param_re)) {
      ...
      for (const auto &payload : payloads) {
        ...
        if (resp.status_code == 200 &&
            (resp.body.find("evil.com") != std::string::npos)) {
          findings.push_back(Finding{"Open Redirect — Client-Side", "low", inject_url + payload,
                              "Server reflects redirect URL in response body (meta/JS redirect).",
                              param, payload, ""});
          return findings;
        }
      }
...
std::vector<Finding> scan_host_injection(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
