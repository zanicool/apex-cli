...
std::vector<Finding> scan_pii_in_errors(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> error_triggers = {"/api/user/999999999", "/api/x'", "/api/null", "/%00"};
  const std::vector<std::string> pii_patterns = {"stack trace", "SQLException", "at com.",  "at org.", "/home/",
                                                 "/var/www/",   "C:\\\\",       "password", "secret"};
    ...
    if (resp.status_code >= 400 && resp.status_code < 600) {
      for (const auto& p : pii_patterns) {
        if (resp.body.find(p) != std::string::npos) {
          findings.push_back(
              {"Verbose Error — Info Leak", "medium", base + path, "Error response exposes: " + p, "", "", resp.body.substr(0, 200)});
          goto next_error;
        }
      }
    }
...
std::vector<Finding> scan_stack_trace(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  auto resp = http.post(base + "/api/" + std::string(500, 'A'), "{invalid json!!", "application/json");
  if (resp.body.find("Traceback") != std::string::npos || resp.body.find("Exception in") != std::string::npos ||
      resp.body.find("at Object.") != std::string::npos || resp.body.find("NullPointerException") != std::string::npos) {
    findings.push_back(
        {"Stack Trace in Production", "medium", base, "Full stack trace exposed in error response", "", "", resp.body.substr(0, 300)});
  }
  return findings;
}

/// Third-party tracking without consent.
std::vector<Finding> scan_tracking_no_consent(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  if (!found_trackers.empty()) {
    ...
    if (resp.body.find("consent") == std::string::npos && resp.body.find("cookie-banner") == std::string::npos &&
      ...
      findings.push_back({"Tracking Without Consent", "medium", crawl.urls[0],
                          "Third-party trackers loaded without consent banner: " + tracker_list, "", "", "GDPR compliance issue"});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_password_storage(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Check if password is returned in user profile API
  const std::vector<std::string> profile_paths = {"/api/me", "/api/user", "/api/profile", "/api/account"};

  for (const auto& path : profile_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200) {
      if (resp.body.find("\"password\"") != std::string::npos || resp.body.find("\"password_hash\"") != std::string::npos) {
        findings.push_back({"Password in API Response", "critical", base + path, "Password or hash returned in user profile endpoint", "",
...
std::vector<Finding> scan_security_txt(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (resp.status_code != 200 || resp.body.find("Contact:") == std::string::npos) {
    findings.push_back({"Missing security.txt", "info", base, "No security.txt — makes responsible disclosure harder", "", "",
...
std::vector<Finding> scan_hsts(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (hsts == resp.headers.end()) {
    findings.push_back({"Missing HSTS", "medium", base, "No Strict-Transport-Security header — downgrade attacks possible", "", "", ""});
    ...
    if (std::regex_search(hsts->second, m, age_re)) {
      ...
      if (age < 31536000) {
        findings.push_back({"HSTS Too Short", "low", base, "HSTS max-age is " + std::to_string(age) + "s (should be >= 31536000)", "", "",
...
std::vector<Finding> scan_mixed_content(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  if (base.find("https://") == std::string::npos) return findings;
  ...
  if (std::regex_search(resp.body, m, http_res_re)) {
    findings.push_back(
        {"Mixed Content", "low", crawl.urls[0], "HTTPS page loads resources over HTTP", "", "", "Found http:// reference in page source"});
  }
  return findings;
}

/// Autocomplete on sensitive fields.
std::vector<Finding> scan_autocomplete(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  for (const auto& url : crawl.urls) {
    auto resp = http.get(url);
    if (resp.body.find("type=\"password\"") != std::string::npos && resp.body.find("autocomplete=\"off\"") == std::string::npos &&
        resp.body.find("autocomplete=\"new-password\"") == std::string::npos) {
      findings.push_back({"Autocomplete on Password", "info", url, "Password field allows browser autocomplete", "", "",
...
std::vector<Finding> scan_cors_wildcard_creds(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  if (acao != resp.headers.end() && acac != resp.headers.end()) {
    if (acao->second == "https://evil.com" && acac->second == "true") {
      findings.push_back({"CORS Credential Theft", "critical", crawl.urls[0], "Arbitrary origin reflected with credentials allowed",
                          "Origin", "https://evil.com", "ACAO: evil.com + ACAC: true"});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_referrer_leak(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
