...
std::vector<Finding> scan_m365_tenant(const Config& cfg, HttpClient& http, const CrawlResult&) {
  ...
  if (oidc.status_code == 200 && oidc.body.find("tenant") != std::string::npos) {
    findings.push_back({"M365 Tenant Detected", "info", domain, "Microsoft 365 / Azure AD tenant exists for this domain", "", "", ""});

    // Extract tenant ID.
    std::regex tid_re(R"(([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}))");
    std::smatch m;
    if (std::regex_search(oidc.body, m, tid_re)) {
      findings.push_back({"Azure AD Tenant ID", "info", domain, "Tenant ID: " + m[0].str(), "", "", ""});
    }
  }
  ...
  if (realm.status_code == 200) {
    if (realm.body.find("Federated") != std::string::npos) {
      findings.push_back(
          {"M365 Federated Auth (ADFS/Okta)", "info", domain, "Domain uses federated authentication — ADFS/Okta/PingFederate", "", "", ""});
    } else if (realm.body.find("Managed") != std::string::npos) {
      findings.push_back({"M365 Managed Auth (Cloud)", "info", domain, "Domain uses cloud-managed authentication", "", "", ""});
    }
    // Check for legacy auth hints.
    if (realm.body.find("NameSpaceType") != std::string::npos) {
      findings.push_back({"M365 Namespace Enumerable", "low", domain, "GetUserRealm endpoint reveals auth configuration", "", "", ""});
    }
  }
  ...
  if (autodiscover.status_code == 401) {
    findings.push_back({"Exchange Autodiscover Active", "info", "autodiscover." + domain,
                        "Exchange autodiscover endpoint exists (401 = requires auth)", "", "", ""});
  } else if (autodiscover.status_code == 200) {
    findings.push_back(
        {"Exchange Autodiscover Open", "high", "autodiscover." + domain, "Autodiscover accessible without auth — config leak", "", "", ""});
  }
...
std::vector<Finding> scan_sharepoint(const Config& cfg, HttpClient& http, const CrawlResult&) {
  std::vector<Finding> findings;
  std::string domain = cfg.target;
  if (domain.find("://") != std::string::npos) domain = domain.substr(domain.find("://") + 3);
  if (domain.find('/') != std::string::npos) domain = domain.substr(0, domain.find('/'));
  ...
  const std::vector<std::string> sp_urls = {
      "https://" + org + ".sharepoint.com",          "https://" + org + "-my.sharepoint.com",
      "https://" + org + ".sharepoint.com/_api/web", "https://" + org + ".sharepoint.com/_api/search/query?querytext='*'",
  ...
  for (const auto& url : sp_urls) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 100) {
      findings.push_back(
          {"SharePoint Accessible", "medium", url, "SharePoint endpoint accessible — check guest/anonymous access", "", "", ""});
    } else if (resp.status_code == 403) {
      findings.push_back({"SharePoint Exists", "info", url, "SharePoint tenant exists (403 = auth required)", "", "", ""});
    }
  }
  ...
  for (const auto& site : {"public", "intranet", "hr", "wiki", "docs", "portal"}) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 500) {
      findings.push_back({"SharePoint Site Public: " + std::string(site), "high", "https://" + org + ".sharepoint.com/sites/" + site,
...
std::vector<Finding> scan_atlassian(const Config& cfg, HttpClient& http, const CrawlResult&) {
  std::vector<Finding> findings;
  std::string domain = cfg.target;
  if (domain.find("://") != std::string::npos) domain = domain.substr(domain.find("://") + 3);
  if (domain.find('/') != std::string::npos) domain = domain.substr(0, domain.find('/'));
  ...
  for (const auto& check : checks) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 50) {
      ...
      if (resp.body.find("displayName") != std::string::npos || resp.body.find("emailAddress") != std::string::npos)
        severity = "high";
      else if (resp.body.find("key") != std::string::npos)
        severity = "medium";
      findings.push_back(
          {check.name + " Exposed", severity, check.url, check.name + " accessible — may leak project/user info", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_slack(const Config& cfg, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  std::string domain = cfg.target;
  if (domain.find("://") != std::string::npos) domain = domain.substr(domain.find("://") + 3);
  if (domain.find('/') != std::string::npos) domain = domain.substr(0, domain.find('/'));
  ...
  if (resp.status_code == 200 && resp.body.find("slack") != std::string::npos) {
    findings.push_back(
        {"Slack Workspace Found", "info", "https://" + org + ".slack.com", "Slack workspace exists for this organization", "", "", ""});
  }
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (page.body.find("hooks.slack.com") != std::string::npos) {
      findings.push_back({"Slack Webhook Leaked", "high", url, "Slack webhook URL in source — can post messages to channels", "", "", ""});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_oauth_enum(const Config& cfg, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  std::string domain = cfg.target;
  if (domain.find("://") != std::string::npos) domain = domain.substr(domain.find("://") + 3);
  if (domain.find('/') != std::string::npos) domain = domain.substr(0, domain.find('/'));
  ...
  std::vector<SSOCheck> checks = {
      {"https://" + org + ".okta.com", "Okta"},    {"https://sso." + domain, "SSO Portal"},
      {"https://login." + domain, "Login Portal"}, {"https://auth." + domain, "Auth Portal"},
      {"https://" + org + ".auth0.com", "Auth0"},  {"https://accounts.google.com/o/saml2?idpid=" + org, "Google SAML"},
  };
  ...
  for (const auto& check : checks) {
    ...
    if (resp.status_code == 200 || resp.status_code == 302) {
      findings.push_back({check.provider + " Detected", "info", check.url, check.provider + " identity provider in use", "", "", ""});
    }
  }
...
std::vector<Finding> scan_cloud_naming(const Config& cfg, HttpClient& http, const CrawlResult&) {
  std::vector<Finding> findings;
  std::string domain = cfg.target;
  if (domain.find("://") != std::string::npos) domain = domain.substr(domain.find("://") + 3);
  if (domain.find('/') != std::string::npos) domain = domain.substr(0, domain.find('/'));
  ...
  for (const auto& res : resources) {
    ...
    if (resp.status_code > 0 && resp.status_code < 500 && resp.error.empty()) {
      findings.push_back(
          {"Cloud Resource: " + res.type, "info", res.url, res.type + " exists — reveals infrastructure naming", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_user_enum(const Config& cfg, HttpClient& http, const CrawlResult&) {
  std::vector<Finding> findings;
  std::string domain = cfg.target;
  if (domain.find("://") != std::string::npos) domain = domain.substr(domain.find("://") + 3);
  if (domain.find('/') != std::string::npos) domain = domain.substr(0, domain.find('/'));
...
std::vector<Scanner> register_enterprise_osint_scanners() {
  return {
      {"M365 Tenant Enum", scan_m365_tenant}, {"SharePoint Exposure", scan_sharepoint}, {"Atlassian (Jira/Confluence)", scan_atlassian},
      {"Slack Discovery", scan_slack},        {"OAuth/SSO Enum", scan_oauth_enum},      {"Cloud Naming Intel", scan_cloud_naming},
