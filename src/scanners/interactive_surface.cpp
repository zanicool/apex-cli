...
std::vector<Finding> scan_login_discovery(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  const std::vector<std::string> login_paths = {
      "/login",         "/signin",          "/sign-in",      "/auth/login", "/admin/login",   "/user/login",
      "/account/login", "/portal",          "/client",       "/clients",    "/dashboard",     "/my-account",
      "/members",       "/member/login",    "/wp-login.php", "/wp-admin",   "/administrator", "/admin",
      "/panel",         "/cpanel",          "/webmail",      "/owa",        "/remote",        "/api/auth/login",
      "/api/login",     "/oauth/authorize", "/sso/login",    "/saml/login", "/cas/login",
  };
  ...
  for (const auto& path : login_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 200) {
      ...
      bool has_password = resp.body.find("password") != std::string::npos || resp.body.find("passwd") != std::string::npos;
      ...
      if (has_password && has_input) {
        findings.push_back({"Login Form Found", "info", base + path, "Authentication form at " + path, "", "", ""});
      } else if (has_form && has_input) {
        findings.push_back({"Interactive Form", "info", base + path, "Form-based interface at " + path, "", "", ""});
      }
    } else if (resp.status_code == 302 || resp.status_code == 301) {
      auto loc = resp.headers.find("Location");
      if (loc != resp.headers.end()) {
        findings.push_back({"Auth Redirect", "info", base + path, "Redirects to: " + loc->second, "", "", ""});
      }
    }
  }
...
std::vector<Finding> scan_interactive_elements(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (auto it = begin; it != end; ++it) {
      ...
      // Categorize the form.
      if (form_tag.find("file") != std::string::npos || form_tag.find("multipart") != std::string::npos ||
    ...
    if (resp.body.find("type=\"file\"") != std::string::npos || resp.body.find("type='file'") != std::string::npos) {
  ...
  if (!found_forms.empty()) {
    ...
    findings.push_back({"Form Actions Discovered", "info", crawl.urls[0], detail, "", "", ""});
  }

  if (!found_uploads.empty()) {
    for (const auto& u : found_uploads) {
      findings.push_back({"File Upload Found", "medium", u, "File upload functionality — test for unrestricted upload", "", "", ""});
    }
  }
  ...
  if (!found_iframes.empty()) {
    for (const auto& src : found_iframes) {
      ...
      if (src.find("http") == 0 && src.find(base_url_from(crawl.urls[0])) == std::string::npos) {
        findings.push_back({"External Iframe", "low", src, "Third-party iframe embedded — verify trust", "", "", ""});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_js_handlers(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (auto it = begin; it != end; ++it) {
      ...
      std::smatch m;
      if (std::regex_search(handler, m, url_in_handler)) event_urls.insert(m[1].str());
    }
  ...
  if (!ajax_endpoints.empty()) {
    ...
    findings.push_back({"AJAX Endpoints in Handlers", "info", crawl.urls[0], detail, "", "", ""});
    ...
    for (const auto& ep : ajax_endpoints) {
      ...
      if (resp.status_code == 200 && resp.body.size() > 20 && resp.body.find("unauthorized") == std::string::npos) {
        findings.push_back(
            {"AJAX Endpoint Accessible", "medium", full, "API endpoint from JS handler accessible without auth", "", "", ""});
      }
...
std::vector<Finding> scan_account_flows(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  // Early return if no URLs to scan.
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& flow : flows) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 200 &&
        (resp.body.find("<form") != std::string::npos || resp.body.find("<input") != std::string::npos ||
         resp.body.find("submit") != std::string::npos)) {
      findings.push_back(
          {std::string(flow.name) + " Flow", "info", base + flow.path, std::string(flow.name) + " form accessible", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_hidden_forms(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (std::regex_search(resp.body, m, hidden_re)) {
      findings.push_back({"Hidden Form", "medium", url, "Hidden form found — may be debug/admin interface", "", "", ""});
    }
    ...
    if (std::regex_search(resp.body, m, comment_re)) {
      ...
      if (comment.find("action") != std::string::npos || comment.find("api") != std::string::npos) {
        findings.push_back({"Commented Code with Endpoints", "low", url, "HTML comment contains endpoint references", "", "", ""});
      }
    }

    // Disabled form fields (may accept input if enabled client-side).
    if (resp.body.find("disabled") != std::string::npos && resp.body.find("<input") != std::string::npos) {
      std::regex disabled_re(R"(<input[^>]*disabled[^>]*name\s*=\s*["']([^"']+)["'])");
      if (std::regex_search(resp.body, m, disabled_re)) {
        findings.push_back({"Disabled Input Field: " + m[1].str(), "low", url, "Disabled field '" + m[1].str() + "' — may be bypassable",
                            m[1].str(), "", ""});
      }
    }
  }
