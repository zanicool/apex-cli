...
std::vector<Finding> scan_docker_registry(const Config &, HttpClient &http,
  ...
  if (resp.status_code == 200 &&
      resp.body.find("repositories") != std::string::npos &&
      resp.body.find("[") != std::string::npos) {
    findings.push_back(Finding{"Docker Registry — Public Access", "critical",
                        base + "/v2/_catalog",
                        ...
                        "", "/v2/_catalog",
                        "Response contains repository listing"});
  }
  return findings;
}

/// Kubernetes API/dashboard exposed.
std::vector<Finding> scan_k8s_exposed(const Config &, HttpClient &http,
                                       const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (dash.status_code == 200 &&
      dash.body.find("\"kind\":\"NamespaceList\"") != std::string::npos) {
    findings.push_back(Finding{"Kubernetes API — Unauthenticated", "critical",
  ...
  if (ui.status_code == 200 &&
      ui.body.find("kubernetes-dashboard") != std::string::npos) {
    findings.push_back(Finding{"Kubernetes Dashboard — Exposed", "critical",
                        base + "/dashboard/",
...
std::vector<Finding> scan_terraform_state(const Config &, HttpClient &http,
                                           const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::vector<std::string> paths = {
      ...
      "/tfstate", "/state.tf", "/main.tfstate"};

  for (const auto &path : paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 &&
        resp.body.find("\"terraform_version\"") != std::string::npos &&
        resp.body.find("\"resources\"") != std::string::npos) {
      findings.push_back(Finding{"Terraform State Exposed", "critical",
                          base + path,
                          ...
                          "", path,
                          "terraform_version found in response"});
      return findings;
    }
  }
  return findings;
...
std::vector<Finding> scan_vault_consul(const Config &, HttpClient &http,
                                        const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (vault.status_code == 200 &&
      vault.body.find("initialized") != std::string::npos &&
      vault.body.find("sealed") != std::string::npos) {
    findings.push_back(Finding{"HashiCorp Vault — Exposed", "high",
                        base + "/v1/sys/health",
                        "Vault health endpoint accessible. Check if unsealed and auth-free.",
                        "", "", vault.body.substr(0, 200)});
  }

  // Consul
  auto consul = http.get(base + "/v1/catalog/services");
  if (consul.status_code == 200 &&
      consul.body.find("{") == 0 &&
      consul.body.size() > 10) {
    findings.push_back(Finding{"HashiCorp Consul — Exposed", "high",
...
std::vector<Finding> scan_aws_misconfig(const Config &, HttpClient &http,
                                         const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (std::regex_search(resp.body, m, cognito_re)) {
    findings.push_back(Finding{"AWS Cognito Identity Pool ID Exposed", "medium",
                        base,
                        "Cognito Identity Pool ID found in client-side code: " + m.str() +
                        ". Test for unauthenticated role access.",
                        "", m.str(), ""});
  }
  ...
  if (std::regex_search(resp.body, m, aws_arn_re)) {
    findings.push_back(Finding{"AWS ARN Leaked", "medium", base,
                        "AWS Account ARN exposed in page: " + m.str(),
                        "", m.str(), ""});
  }
...
std::vector<Finding> scan_cloud_config_leak(const Config &, HttpClient &http,
                                             const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto &check : checks) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 20 &&
      ...
      if (check.indicator.empty() ||
        ...
        findings.push_back(Finding{check.name + " Exposed", "high",
                            base + check.path,
                            check.name + " publicly accessible. May contain credentials.",
                            "", check.path,
...
std::vector<Finding> scan_cicd_exposure(const Config &, HttpClient &http,
                                         const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto &check : checks) {
    auto resp = http.get(base + check.path);
    if (resp.status_code == 200 && resp.body.size() > 30 &&
        resp.body.find(check.indicator) != std::string::npos &&
        resp.body.find("<!DOCTYPE") == std::string::npos &&
        resp.body.find("Access Denied") == std::string::npos) {
      findings.push_back(Finding{check.name + " Exposed", "medium",
                          base + check.path,
...
std::vector<Scanner> register_cloud_attack_scanners() {
  return {
      ...
      {"Kubernetes Exposed", scan_k8s_exposed},
      {"Terraform State", scan_terraform_state},
      {"Vault/Consul", scan_vault_consul},
      {"AWS Misconfig", scan_aws_misconfig},
      {"Cloud Config Leak", scan_cloud_config_leak},
