...
const KernelCVE kernel_cves[] = {
    {"CVE-2026-31431", "Copy Fail", "high", "Local privilege escalation via copy_file_range — 732-byte exploit", 5, 0, 6, 14},
    {"CVE-2026-43284", "Dirty Frag (part 1)", "high", "IP fragment handling bug — combined with CVE-2026-43500 for root", 5, 4, 6, 14},
    {"CVE-2026-43500", "Dirty Frag (part 2)", "high", "Netfilter fragment reassembly — single command root", 5, 4, 6, 14},
    {"CVE-2026-46300", "Fragnesia", "high", "Kernel memory corruption via fragmented packets", 5, 10, 6, 14},
    {"CVE-2026-46333", "ssh-keysign-pwn", "high", "Local privilege escalation via ssh-keysign", 5, 0, 6, 14},
    // Older but still common
    {"CVE-2024-1086", "nf_tables use-after-free", "high", "Netfilter nf_tables local privilege escalation", 5, 0, 6, 7},
    {"CVE-2023-32233", "nf_tables batch", "high", "Netfilter nf_tables batch request UAF", 5, 0, 6, 3},
};

/// Parse kernel version string like "6.8.0-45-generic".
bool parse_kernel(const std::string& ver, int& major, int& minor, int& patch) {
  if (sscanf(ver.c_str(), "%d.%d.%d", &major, &minor, &patch) >= 2) return true;
  return false;
}
...
std::vector<Finding> scan_ssh_audit(const Config& cfg, HttpClient&, const CrawlResult&) {
  ...
  if (!cfg.ssh_key.empty()) ssh_prefix += "-i " + cfg.ssh_key + " ";
  ...
  findings.push_back({"Kernel Version", "info", cfg.ssh_target, "Kernel: " + uname, "", "", ""});
  ...
  for (const auto& cve : kernel_cves) {
    if (is_vulnerable(major, minor, cve)) {
      findings.push_back({std::string(cve.cve) + " (" + cve.name + ")", cve.severity, cfg.ssh_target,
  ...
  if (!distro.empty()) {
    ...
    if (eq != std::string::npos) {
      ...
      findings.push_back({"OS Distribution", "info", cfg.ssh_target, "OS: " + name, "", "", ""});
    }
  }
  ...
  if (autoupdate.find("active") == std::string::npos) {
    findings.push_back({"No Auto-Updates", "medium", cfg.ssh_target, "Automatic security updates not enabled", "", "", ""});
  }
  ...
  if (services.find(":6379") != std::string::npos) {
    findings.push_back({"Redis Exposed", "high", cfg.ssh_target + ":6379", "Redis listening — check if auth is required", "", "", ""});
  }
  if (services.find(":27017") != std::string::npos) {
    findings.push_back({"MongoDB Exposed", "high", cfg.ssh_target + ":27017", "MongoDB listening — check if auth is required", "", "", ""});
  }
  if (services.find(":9200") != std::string::npos) {
    findings.push_back(
        {"Elasticsearch Exposed", "high", cfg.ssh_target + ":9200", "Elasticsearch listening — check if auth is required", "", "", ""});
  }
