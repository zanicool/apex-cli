...
std::vector<Finding> scan_zone_transfer(const Config &cfg, HttpClient &,
                                         const CrawlResult &) {
  std::vector<Finding> findings;
  std::string cmd = "dig @$(dig +short NS " + cfg.target +
                    " | head -1) " + cfg.target + " AXFR +short 2>/dev/null | head -5";
  ...
  if (!output.empty() && output.find("failed") == std::string::npos &&
      output.find("Transfer failed") == std::string::npos &&
      output.size() > 10) {
    findings.push_back({"DNS Zone Transfer", "high", cfg.target,
                        "AXFR zone transfer allowed — full DNS records exposed",
                        "", "", output.substr(0, 200)});
  }
  return findings;
}
...
std::vector<Finding> scan_debug_endpoints(const Config &, HttpClient &http,
  ...
  for (const auto &[path, name] : debug_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 100) {
      ...
      if (path.find("heapdump") != std::string::npos ||
          path.find("jolokia") != std::string::npos ||
          path.find("console") != std::string::npos)
        sev = "critical";
      findings.push_back({"Debug Endpoint Exposed: " + name, sev, base + path,
  ...
  for (const auto &cred : creds) {
    ...
    if (resp.status_code == 200 &&
        (resp.body.find("token") != std::string::npos ||
         resp.body.find("session") != std::string::npos ||
      ...
      findings.push_back({"Default Credentials", "critical", base + cred.path,
                          "Login succeeded with " + cred.name,
                          "", cred.name,
...
std::vector<Finding> scan_backup_files(const Config &, HttpClient &http,
  ...
  for (const auto &[path, type] : backup_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 20) {
      ...
      if (resp.body.find("<html") != std::string::npos &&
          resp.body.find("not found") != std::string::npos) continue;

      std::string sev = "high";
      if (path.find(".sql") != std::string::npos ||
          path.find(".env") != std::string::npos ||
          path.find("htpasswd") != std::string::npos)
        sev = "critical";

      findings.push_back({"Backup/Source Exposed: " + path, sev, base + path,
                          type + " file publicly accessible",
                          "", "", std::to_string(resp.body.size()) + " bytes"});
    }
  }
  return findings;
...
std::vector<Finding> scan_directory_listing(const Config &, HttpClient &http,
                                             const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> dirs = {
      ...
      "/files/", "/media/", "/tmp/", "/backup/"};

  for (const auto &dir : dirs) {
    auto resp = http.get(base + dir);
    if (resp.status_code == 200 &&
        (resp.body.find("Index of") != std::string::npos ||
         resp.body.find("Directory listing") != std::string::npos ||
         resp.body.find("<pre>") != std::string::npos)) {
      findings.push_back({"Directory Listing", "low", base + dir,
                          "Directory contents exposed to public",
                          "", "", ""});
    }
  }
  return findings;
...
std::vector<Finding> scan_cookie_security(const Config &, HttpClient &http,
  ...
  for (const auto &[header, value] : resp.headers) {
    ...
    bool is_session = lower.find("session") != std::string::npos ||
    ...
    if (lower.find("httponly") == std::string::npos) {
      findings.push_back({"Cookie Missing HttpOnly", "medium", crawl.urls[0],
                          "Session cookie accessible via JavaScript",
                          "", "", value.substr(0, 60)});
    }
    if (lower.find("secure") == std::string::npos) {
      findings.push_back({"Cookie Missing Secure Flag", "medium", crawl.urls[0],
                          "Session cookie sent over HTTP",
                          "", "", value.substr(0, 60)});
    }
    if (lower.find("samesite") == std::string::npos) {
      findings.push_back({"Cookie Missing SameSite", "low", crawl.urls[0],
...
std::vector<Finding> scan_path_traversal(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  const std::vector<std::string> traversal_params = {
      "file", "path", "page", "include", "template", "doc", "folder", "lang"};
    ...
    for (const auto &param : traversal_params) {
      ...
      if (resp.status_code == 200 &&
          (resp.body.find("root:") != std::string::npos ||
           resp.body.find("[extensions]") != std::string::npos)) {
        findings.push_back({"Path Traversal", "critical", url,
                            "Local file read via " + param + " parameter",
                            param, payload,
...
std::vector<Finding> scan_server_disclosure(const Config &, HttpClient &http,
                                             const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  if (server != resp.headers.end() && server->second.size() > 3) {
    ...
    if (std::regex_search(server->second, ver_re)) {
      findings.push_back({"Server Version Disclosure", "low", crawl.urls[0],
                          "Server header exposes version: " + server->second,
  ...
  if (powered != resp.headers.end()) {
    findings.push_back({"Technology Disclosure", "low", crawl.urls[0],
                        "X-Powered-By header exposes: " + powered->second,
...
std::vector<Scanner> register_infra_misconfig_scanners() {
  return {
      {"DNS Zone Transfer", scan_zone_transfer},
      ...
      {"Path Traversal (Deep)", scan_path_traversal},
      {"Server Disclosure", scan_server_disclosure},
  };
}
