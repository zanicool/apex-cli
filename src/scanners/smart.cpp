...
[[maybe_unused]] static bool detect_waf(HttpClient& http, const std::string& url) {
  ...
  const std::vector<std::string> waf_sigs = {"cloudflare",   "akamai",  "incapsula", "sucuri", "wordfence",
...
bool is_wildcard(HttpClient& http, const std::string& url, const std::string& param) {
  ...
  size_t diff = (r1.body.size() > r2.body.size()) ? r1.body.size() - r2.body.size() : r2.body.size() - r1.body.size();
  return diff < 10 && r1.status_code == r2.status_code;
}

/// Boolean-blind SQL injection via response size differential.
std::vector<Finding> scan_sqli_blind_boolean(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (const auto& [base, param] : targets) {
      ...
      size_t size_diff = (true_resp.body.size() > false_resp.body.size()) ? true_resp.body.size() - false_resp.body.size()
                                                                          : false_resp.body.size() - true_resp.body.size();
      if (size_diff > 50 && true_resp.status_code == false_resp.status_code) {
        findings.push_back(
            {"SQLi (Blind)", "high", url, "Boolean-blind SQLi: size diff=" + std::to_string(size_diff), param, true_payload, ""});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_xss_context_aware(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (const auto& [base, param] : targets) {
      ...
      if (pos > 0) {
        std::string before = resp.body.substr(pos > 30 ? pos - 30 : 0, pos > 30 ? 30 : pos);
        if (before.find("=\"") != std::string::npos || before.find("='") != std::string::npos)
          ctx = "attr";
        else if (before.find("var ") != std::string::npos || before.find("'") != std::string::npos)
          ctx = "js";
      ...
      for (const auto& cp : ctx_payloads) {
        ...
        if (test.body.find(cp.payload) != std::string::npos) {
          findings.push_back({"XSS (Context)", "high", url, "Context-aware XSS in " + ctx + " context", param, cp.payload, ""});
          break;
        }
      }
    }
...
std::vector<Finding> scan_waf_detect(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  const WafSig sigs[] = {
      {"Server", "cloudflare", "Cloudflare"},      {"X-Sucuri-ID", "", "Sucuri"},
      {"X-CDN", "Incapsula", "Imperva/Incapsula"}, {"Server", "AkamaiGHost", "Akamai"},
  ...
  for (const auto& sig : sigs) {
    ...
    if (it != resp.headers.end()) {
      ...
      if (needle.empty() || it->second.find(needle) != std::string::npos) {
        findings.push_back({"WAF Detected", "info", base, std::string("WAF: ") + sig.name, "", "", ""});
      }
    }
  }
  ...
  if (resp.status_code == 403 || resp.status_code == 406) {
    findings.push_back(
        {"WAF Detected", "info", base, "WAF blocking malicious input (HTTP " + std::to_string(resp.status_code) + ")", "", "", ""});
  }
  return findings;
}

/// Time-based blind SQL injection.
std::vector<Finding> scan_sqli_time_based(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  const std::vector<std::string> payloads = {"' AND SLEEP(5)--", "'; WAITFOR DELAY '0:0:5'--", "' AND pg_sleep(5)--", "1; SELECT SLEEP(5)"};
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (const auto& [base, param] : targets) {
      ...
      for (const auto& payload : payloads) {
        ...
        if (resp.duration.count() >= 4500 && baseline.duration.count() < 2000) {
          findings.push_back(
              {"SQLi (Time)", "high", url, "Time-based blind SQLi: " + std::to_string(resp.duration.count()) + "ms", param, payload, ""});
          break;
        }
      }
    }
