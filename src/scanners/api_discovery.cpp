...
std::vector<Finding> scan_openapi_exposure(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& ep : endpoints) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 100 && resp.body.find(ep.indicator) != std::string::npos) {
      ...
      findings.push_back({"API Docs Exposed: " + std::string(ep.name), "high", base + ep.path, detail, "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_graphql_discovery(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> gql_paths = {
      "/graphql",        "/graphql/",        "/api/graphql", "/v1/graphql", "/gql",    "/query",   "/graphql/v1",
      "/api/v1/graphql", "/graphql/console", "/playground",  "/graphiql",   "/altair", "/voyager", "/graphql-explorer",
  };

  const std::string introspection_query = R"({"query":"{ __schema { queryType { name } mutationType { name } types { name kind } } }"})";
  ...
  for (const auto& path : gql_paths) {
    ...
    if (resp.status_code == 200 && resp.body.find("__schema") != std::string::npos) {
      ...
      findings.push_back({"GraphQL Introspection", "high", base + path,
                          "Full schema exposed via introspection — " + std::to_string(types) + " types discovered", "", "", ""});
      break;
    }
    // Also check GET-based GraphQL explorers.
    auto get_resp = http.get(base + path);
    if (get_resp.status_code == 200 &&
        (get_resp.body.find("graphiql") != std::string::npos || get_resp.body.find("GraphQL Playground") != std::string::npos ||
         get_resp.body.find("apollo-sandbox") != std::string::npos || get_resp.body.find("voyager") != std::string::npos)) {
      findings.push_back({"GraphQL Explorer Exposed", "high", base + path, "Interactive GraphQL explorer publicly accessible", "", "", ""});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_api_gateway(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& gw : gateways) {
    ...
    if (gw.path[0] == '/' && gw.path[1] == ':') {
      ...
      std::string host = (host_end != std::string::npos) ? base.substr(0, host_end) : base;
      url = host + port + path;
    }
    auto resp = http.get(url);
    if (resp.status_code == 200 && resp.body.size() > 50 && resp.body.find(gw.indicator) != std::string::npos) {
      findings.push_back({"API Gateway Exposed: " + std::string(gw.gateway), "critical", url,
...
std::vector<Finding> scan_api_enumeration(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> discovery_paths = {
      "/", "/health", "/status", "/info", "/version", "/users", "/me", "/config", "/settings",
  };
  ...
  for (const auto& api : api_bases) {
    for (const auto& path : discovery_paths) {
      ...
      if (resp.status_code == 200 && resp.body.size() > 20 && (resp.body[0] == '{' || resp.body[0] == '[')) {
  ...
  if (!found_apis.empty()) {
    ...
    findings.push_back({"API Enumeration", "info", base, detail, "", "", ""});
  }
  ...
  if (odata.status_code == 200 && odata.body.find("edmx") != std::string::npos) {
    findings.push_back(
        {"OData Metadata Exposed", "medium", base + "/odata/$metadata", "OData $metadata endpoint exposes full entity model", "", "", ""});
  }

  // Check for WSDL (SOAP).
  auto wsdl = http.get(base + "/service?wsdl");
  if (wsdl.status_code == 200 && wsdl.body.find("definitions") != std::string::npos) {
    findings.push_back({"WSDL Exposed", "medium", base + "/service?wsdl", "SOAP WSDL exposes all operations and types", "", "", ""});
  }
  ...
  if (grpc.status_code != 404 && grpc.body.size() > 0) {
    findings.push_back({"gRPC Reflection", "medium", base, "gRPC server reflection may be enabled", "", "", ""});
  }
