...
std::vector<Finding> scan_api_key_in_url(const Config&, HttpClient&, const CrawlResult& crawl) {
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (std::regex_search(url, m, key_re)) {
      findings.push_back({"API Key in URL", "medium", url, "Sensitive token in query string — logged in server/proxy logs", m[1].str(),
                          m[2].str().substr(0, 10) + "...", "Key param found in crawled URL"});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_excessive_data(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  const std::vector<std::string> sensitive_fields = {"\"password\"",    "\"password_hash\"", "\"secret\"", "\"ssn\"",
  ...
  for (const auto& url : crawl.urls) {
    ...
    for (const auto& field : sensitive_fields) {
      if (resp.body.find(field) != std::string::npos) {
        findings.push_back({"Excessive Data Exposure", "high", url, "API response contains sensitive field: " + field, "", field,
...
std::vector<Finding> scan_bola(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (r1.status_code == 200 && r2.status_code == 200 && r1.body != r2.body && contains_any(r2.body, pii)) {
      findings.push_back({"BOLA (Broken Object Level Auth)", "high", url, "Different user objects accessible by incrementing ID", "path_id",
                          std::to_string(id + 1), "Response contains PII for adjacent ID"});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_dangerous_methods(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (allow != resp.headers.end()) {
      if (allow->second.find("DELETE") != std::string::npos || allow->second.find("PUT") != std::string::npos) {
        findings.push_back(
            {"Dangerous HTTP Methods", "medium", url, "PUT/DELETE methods allowed: " + allow->second, "", "", "Allow: " + allow->second});
        break;
      }
    }
  }
...
std::vector<Finding> scan_deeplink_hijack(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (resp.status_code == 200 && resp.body.find("target") != std::string::npos) {
    ...
    if (resp.body.find("*") != std::string::npos) {
      findings.push_back({"Deeplink Hijack — Wildcard", "high", base + "/.well-known/assetlinks.json",
                          "assetlinks.json allows wildcard package — any app can claim links", "", "*", resp.body.substr(0, 200)});
    }
  }
  ...
  if (apple.status_code == 200 && apple.body.find("applinks") != std::string::npos) {
    if (apple.body.find("\"*\"") != std::string::npos) {
      findings.push_back({"Universal Link Hijack", "medium", base + "/.well-known/apple-app-site-association",
                          "Apple app-site-association has overly broad path matching", "", "", apple.body.substr(0, 200)});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_ssrf_params(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 50 && resp.body.find("error") == std::string::npos) {
      findings.push_back({"SSRF via URL Parameter", "high", url, "Server fetches attacker-controlled URL", param, "http://127.0.0.1:80",
...
std::vector<Finding> scan_missing_csrf(const Config&, HttpClient&, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& form : crawl.forms) {
    ...
    for (const auto& field : form.fields) {
      ...
      if (lower.find("csrf") != std::string::npos || lower.find("token") != std::string::npos ||
          lower.find("_token") != std::string::npos || lower.find("nonce") != std::string::npos) {
        has_csrf = true;
        break;
      }
    }
    if (!has_csrf && (form.action.find("delete") != std::string::npos || form.action.find("update") != std::string::npos ||
                      form.action.find("admin") != std::string::npos || form.action.find("settings") != std::string::npos)) {
      findings.push_back({"Missing CSRF Token", "medium", form.action, "State-changing form without CSRF protection", "", "",
...
std::vector<Finding> scan_bfla(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> admin_apis = {"/api/admin/users", "/api/admin/config",   "/api/admin/settings",  "/api/admin/logs",
                                               "/api/admin/stats", "/api/internal/debug", "/api/v1/admin/export", "/api/management/users"};
  ...
  for (const auto& path : admin_apis) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 100 && resp.body.find("error") == std::string::npos &&
      ...
      findings.push_back({"BFLA (Broken Function Level Auth)", "critical", base + path, "Admin API accessible without elevated privileges",
                          "", "", "Response contains sensitive data (" + std::to_string(resp.body.size()) + " bytes)"});
    }
  }
  return findings;
}
