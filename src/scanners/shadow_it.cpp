...
std::vector<Finding> scan_shadow_saas(const Config &cfg, HttpClient &http,
                                      const CrawlResult &) {
  ...
  for (const auto &s : shadow_subs) {
    ...
    if (resp.status_code >= 200 && resp.status_code < 500 &&
        resp.status_code != 404 && resp.body.size() > 50) {
      findings.push_back({"Shadow IT: " + std::string(s.service), "medium", url,
                          std::string(s.service) + " detected — risk: " + s.risk,
                          "", "", ""});
    }
  }
  return findings;
...
std::vector<Finding> scan_oauth_sprawl(const Config &cfg, HttpClient &http,
  ...
  const std::vector<std::string> oauth_paths = {
      "/oauth/applications", "/.well-known/openid-configuration",
      "/api/v1/oauth/apps", "/admin/oauth", "/oauth/authorized_applications",
      "/settings/applications", "/api/connected-apps",
  };
  for (const auto &path : oauth_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && resp.body.size() > 100 &&
        (resp.body.find("client_id") != std::string::npos ||
         resp.body.find("redirect_uri") != std::string::npos ||
  ...
  const std::vector<std::string> risky_scopes = {
  ...
  for (const auto &scope : risky_scopes) {
    if (home.body.find(scope) != std::string::npos) {
      findings.push_back({"Risky OAuth Scope", "high", base,
                          "Overly permissive OAuth scope in source: " + scope,
                          "", "", ""});
    }
  }
  ...
  if (tenant.status_code == 200 &&
      tenant.body.find("authorization_endpoint") != std::string::npos) {
    findings.push_back({"M365 Tenant (OAuth target)", "info",
...
std::vector<Finding> scan_exposed_dashboards(const Config &, HttpClient &http,
                                             const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto &d : dashboards) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 200 &&
        resp.body.find(d.indicator) != std::string::npos) {
      findings.push_back({"Exposed Dashboard: " + std::string(d.name), "high",
                          base + d.path,
...
std::vector<Finding> scan_ai_leakage(const Config &cfg, HttpClient &http,
                                      const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::pair<std::string, std::string>> ai_paths = {
      {"/api/chat", "Internal chat API (possible AI proxy)"},
      {"/api/v1/chat/completions", "OpenAI-compatible API endpoint"},
      {"/v1/chat/completions", "OpenAI-compatible API endpoint"},
      {"/api/generate", "Ollama/local LLM endpoint"},
      {"/api/embeddings", "Embedding API (data ingestion)"},
  ...
  for (const auto &[path, desc] : ai_paths) {
    ...
    if (resp.status_code == 200 || resp.status_code == 401 ||
      ...
      findings.push_back({"AI Endpoint Exposed", "high", base + path,
                          desc + " — possible corporate data flowing to AI",
                          "", "", ""});
    }
  }

  // Check for MCP (Model Context Protocol) servers.
  const std::vector<std::string> mcp_paths = {
      "/mcp", "/api/mcp", "/.well-known/mcp.json"};
  ...
  const std::vector<std::string> ai_subs = {
      "openai", "claude", "llm", "chatbot", "assistant", "ollama"};
  for (const auto &sub : ai_subs) {
    auto resp = http.get("https://" + sub + "." + domain + "/");
    if (resp.status_code >= 200 && resp.status_code < 500 &&
        resp.status_code != 404 && resp.body.size() > 50) {
      findings.push_back({"Shadow AI Service", "medium",
                          "https://" + sub + "." + domain,
...
std::vector<Finding> scan_orphaned_resources(const Config &cfg, HttpClient &http,
                                             const CrawlResult &) {
  std::vector<Finding> findings;
  std::string domain = get_domain(cfg);
  std::string org = domain.substr(0, domain.find('.'));
  ...
  const CloudPattern patterns[] = {
      ...
      {"https://%s.blob.core.windows.net/test?restype=container&comp=list",
       "Rogue Azure Blob (test)", "<Blob>"},
      {"https://%s.blob.core.windows.net/temp?restype=container&comp=list",
       "Rogue Azure Blob (temp)", "<Blob>"},
  };
  ...
  for (const auto &p : patterns) {
    ...
    if (resp.status_code == 200 && resp.body.find(p.indicator) != std::string::npos) {
      findings.push_back({"Orphaned Cloud Resource", "high", url,
                          std::string(p.name) + " — publicly listable, likely unmanaged",
                          "", "", ""});
    }
  }
  ...
  const std::vector<std::pair<std::string, std::string>> deploy_patterns = {
      ...
      {"https://" + org + ".vercel.app", "Vercel"},
      {"https://" + org + ".netlify.app", "Netlify"},
      {"https://" + org + ".azurewebsites.net", "Azure App Service"},
      {"https://" + org + ".firebaseapp.com", "Firebase"},
      {"https://" + org + ".web.app", "Firebase"},
  };
...
std::vector<Scanner> register_shadow_it_scanners() {
  return {
      {"Shadow SaaS/Subdomains", scan_shadow_saas},
      {"OAuth Sprawl", scan_oauth_sprawl},
      {"Exposed Dashboards", scan_exposed_dashboards},
      {"AI Leakage Detection", scan_ai_leakage},
