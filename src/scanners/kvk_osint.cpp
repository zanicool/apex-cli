...
std::vector<Finding> scan_kvk_recon(const Config& cfg, HttpClient& http, const CrawlResult&) {
  ...
  for (auto it = std::sregex_iterator(resp.body.begin(), resp.body.end(), kvk_re); it != end_it; ++it)
    kvk_numbers.push_back((*it)[1].str());
  for (auto it = std::sregex_iterator(resp.body.begin(), resp.body.end(), name_re); it != end_it; ++it) names.push_back((*it)[1].str());

  if (kvk_numbers.empty()) return findings;

  findings.push_back({"KVK Company Found", "info", domain,
                      "Found " + std::to_string(kvk_numbers.size()) + " KVK registrations matching '" + org + "'", "", "", ""});
  ...
  for (size_t i = 0; i < limit; ++i) {
    ...
    if (std::regex_search(detail.body, m, sbi_re)) info += " | Activiteit: " + m[1].str();

    std::regex emp_re(R"re("totaalWerkzamePersonen"\s*:\s*(\d+))re");
    if (std::regex_search(detail.body, m, emp_re)) info += " | Werknemers: " + m[1].str();

    std::regex form_re(R"re("rechtsvorm"\s*:\s*"([^"]+)")re");
    if (std::regex_search(detail.body, m, form_re)) info += " | Rechtsvorm: " + m[1].str();
    ...
    int branches = 0;
    for (auto it = std::sregex_iterator(detail.body.begin(), detail.body.end(), vest_re); it != end_it; ++it) branches++;
    if (branches > 1) {
      findings.push_back(
          {"KVK Multiple Branches", "info", domain, std::to_string(branches) + " vestigingen — larger attack surface", "", "", ""});
    }

    // Find directors.
    std::regex func_re(R"re("volledigeNaam"\s*:\s*"([^"]+)")re");
    std::vector<std::string> directors;
    for (auto it = std::sregex_iterator(detail.body.begin(), detail.body.end(), func_re); it != end_it; ++it) {
