...
std::vector<Finding> scan_cache_poison_headers(const Config &, HttpClient &http,
  ...
  std::vector<std::pair<std::string, std::string>> poison_headers = {
      {"X-Forwarded-Host", buster + ".evil.com"},
      {"X-Original-URL", "/" + buster},
      {"X-Rewrite-URL", "/" + buster},
      {"X-Forwarded-Scheme", "nothttps"},
      {"X-Forwarded-Proto", "nothttps"},
      {"X-Host", buster + ".evil.com"},
      {"X-Forwarded-Server", buster + ".evil.com"},
      {"X-HTTP-Method-Override", "POST"},
      {"X-Original-Host", buster + ".evil.com"},
  };
  ...
  for (const auto &[header, value] : poison_headers) {
    ...
    if (resp.status_code == 200 && resp.body.find(value) != std::string::npos) {
      ...
      for (const auto &[key, val] : resp.headers) {
        ...
        if (lower == "cache-control" && (val.find("public") != std::string::npos ||
                                          val.find("max-age") != std::string::npos)) {
          cacheable = true;
        }
        if (lower == "x-cache" || lower == "cf-cache-status" ||
            lower == "x-varnish" || lower == "age") {
          cacheable = true;
        }
      }
      ...
      findings.push_back({"Web Cache Poisoning — " + header, severity, test_url,
                          ...
                          header, value,
                          "Reflected in " + std::to_string(resp.body.size()) + " byte response"});
...
std::vector<Finding> scan_cache_deception(const Config &, HttpClient &http,
  ...
  std::vector<std::string> auth_paths = {
      ...
      "/api/me", "/api/user", "/my-account"};

  std::vector<std::string> static_suffixes = {
      "/apex-test.css", "/apex-test.js", "/apex-test.png",
      "/.css", "/..%2fapex.css", "%0d%0a.css"};
  ...
  for (const auto &path : auth_paths) {
    ...
    for (const auto &suffix : static_suffixes) {
      ...
      if (deception.status_code == 200 && deception.body == normal.body) {
        ...
        findings.push_back({"Web Cache Deception", cached ? "high" : "medium",
                            base + path + suffix,
                            ...
                            "", suffix,
                            cached ? "CONFIRMED: Response was cached" : "Response identical, cache status unknown"});
...
std::vector<Finding> scan_prototype_pollution(const Config &, HttpClient &http,
  ...
  for (; it != end; ++it) {
    findings.push_back({"Prototype Pollution — Vulnerable Library", "medium", base,
                        "Library '" + (*it)[1].str() + " v" + (*it)[2].str() + "' detected. "
...
std::vector<Finding> scan_dom_clobbering(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::regex global_ref_re(R"x((?:window\.|document\.)(config|settings|data|options|params|user|auth|token|key|secret|endpoint|api|url|callback))x");
  ...
  if (!clobberable.empty()) {
    findings.push_back({"DOM Clobbering — Clobberable Globals", "medium", base,
                        "JavaScript references global variables that can be overridden via DOM: " +
                        clobberable[0] + (clobberable.size() > 1 ? " (+" + std::to_string(clobberable.size() - 1) + " more)" : "") +
                        ". If attacker can inject HTML (e.g., via stored content), they can "
                        "override these globals with <a id=\"" + clobberable[0] + "\" href=\"evil\">.",
                        "", clobberable[0], ""});
  }
...
std::vector<Finding> scan_cors_misconfig(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto &[origin, desc] : origins) {
    ...
    if (acao != resp.headers.end() && acao->second == origin) {
      ...
      findings.push_back({"CORS Misconfiguration — " + desc, severity, base,
                          ...
                          "Origin", origin,
                          "ACAO: " + acao->second + (with_creds ? ", ACAC: true" : "")});
      break;
    }
  }
...
std::vector<Scanner> register_cache_poison_scanners() {
  return {
      {"Cache Poison Headers", scan_cache_poison_headers},
      {"Cache Deception", scan_cache_deception},
      {"Prototype Pollution", scan_prototype_pollution},
      {"DOM Clobbering", scan_dom_clobbering},
