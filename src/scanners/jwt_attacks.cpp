...
std::vector<Finding> scan_jwt_analysis(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Try login/auth endpoints to get a JWT
  std::vector<std::string> auth_paths = {"/api/auth/login", "/api/v1/auth/login", "/api/login",  "/api/token",
                                         "/api/v1/token",   "/oauth/token",       "/api/session"};
  ...
  findings.push_back(Finding{"JWT Token Found", "info", base, "JWT detected in responses. Header: " + header_json.substr(0, 200), "", "",
  ...
  if (header_json.find("\"jku\"") != std::string::npos || header_json.find("\"x5u\"") != std::string::npos) {
  ...
  if (payload_json.find("\"password\"") != std::string::npos || payload_json.find("\"secret\"") != std::string::npos ||
...
std::vector<Finding> scan_jwt_none_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Look for auth endpoints that return data
  std::vector<std::string> protected_paths = {"/api/me", "/api/v1/me", "/api/user", "/api/profile", "/api/account", "/api/v1/user/profile"};
  ...
  for (const auto& path : protected_paths) {
    ...
    if (resp.status_code == 401 || resp.status_code == 403) {
      ...
      auto test = http.get(base + path, {{"Authorization", "Bearer " + forged}});
