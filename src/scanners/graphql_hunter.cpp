...
static std::vector<Finding> scan_graphql_hunter(const Config &cfg, HttpClient &http, const CrawlResult &crawl) {
    ...
    std::vector<std::string> prefixes = {
        "api.", "api.dev.", "api.stage.", "api.staging.", "api.test.",
        "graphql.", "gql.", "dev.", "staging.", "stage.", "internal.",
    };

    // GraphQL paths to try on each host
    std::vector<std::string> gql_paths = {
        "/graphql/", "/graphql", "/api/graphql/", "/api/graphql",
        "/gql/", "/gql", "/query", "/v1/graphql",
    };
    ...
    for (const auto &host : hosts) {
        for (const auto &path : gql_paths) {
            ...
            if (resp.body.find("__typename") == std::string::npos &&
                resp.body.find("errors") == std::string::npos &&
                resp.body.find("data") == std::string::npos &&
                resp.body.find("invalid") == std::string::npos) continue;
            ...
            if (intro_resp.body.find("queryType") != std::string::npos) {
                ...
                findings.push_back({"GraphQL Introspection Enabled", "high", url,
            ...
            if (users_resp.body.find("\"data\"") != std::string::npos &&
                users_resp.body.find("username") != std::string::npos) {
                has_unauth_queries = true;
                findings.push_back({"GraphQL User Enumeration (No Auth)", "medium", url, "Users query returns data without authentication", "", "", users_resp.body.substr(0, 200)});
            }

            // Fuzz mutations
            std::vector<std::string> mutations = {
                "createListing", "updateListing", "deleteListing",
                "createOrder", "cancelOrder", "updateOrder",
                "followUser", "unfollowUser", "blockUser",
                "updateProfile", "changePassword", "resetPassword",
                "createAuction", "placeBid", "cancelBid",
                "withdraw", "transfer", "addPaymentMethod",
                "sendMessage", "deleteMessage",
                "createStream", "startStream", "endStream",
            };
...
std::vector<Scanner> register_graphql_hunter() {
    return {{"GraphQL Hunter", scan_graphql_hunter}};
}
