...
const std::vector<std::string> INTERNAL_PAYLOADS = {
    "http://127.0.0.1",  "http://localhost", "http://0.0.0.0", "http://[::1]", "http://[::ffff:127.0.0.1]",
...
std::vector<std::pair<std::string, std::string>> find_url_params(const CrawlResult& crawl) {
  ...
  std::regex url_param_re(
      R"x([?&](url|uri|link|next|redirect|path|src|dest|target|return|callback|webhook|fetch|load|proxy|img|image|avatar|thumbnail|file|document|pdf|page|goto|continue|ref|data|feed|rss)=)x");
...
std::vector<Finding> scan_ssrf_internal(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& [base_url, param] : targets) {
    ...
    for (const auto& [meta_url, cloud] : METADATA_URLS) {
      ...
      if (resp.status_code == 200 && resp.body.size() > 20 &&
          (resp.body.find("ami-") != std::string::npos || resp.body.find("instance") != std::string::npos ||
           resp.body.find("project") != std::string::npos || resp.body.find("compute") != std::string::npos)) {
        findings.push_back({"SSRF — Cloud Metadata Access", "critical", base_url + meta_url,
                            "Server fetches " + cloud +
                                " metadata endpoint. "
                                "Full cloud account compromise possible via instance credentials.",
                            param, meta_url, "Response: " + resp.body.substr(0, 200)});
    ...
    for (const auto& payload : INTERNAL_PAYLOADS) {
      ...
      if (resp.status_code == 200 && resp.body.size() > 50 && resp.body.find("404") == std::string::npos &&
          resp.body.find("error") == std::string::npos && resp.body.find("blocked") == std::string::npos) {
        findings.push_back({"SSRF — Internal Network Access", "high", base_url + payload,
                            "Server fetches internal URL. Attacker can scan internal services.", param, payload,
...
std::vector<Finding> scan_ssrf_redirect(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  auto targets = find_url_params(crawl);
  if (targets.empty()) return findings;
  ...
  for (const auto& [base_url, param] : targets) {
    ...
    if (resp.status_code == 200 || resp.body.find("httpbin") != std::string::npos) {
      findings.push_back({"SSRF — Follows External URLs", "medium", base_url,
                          ...
                          param, "https://httpbin.org/status/200", "Server successfully fetched external URL"});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_ssrf_webhook(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Look for webhook configuration endpoints
  std::vector<std::string> webhook_paths = {"/api/webhooks",     "/api/v1/webhooks", "/webhooks/configure", "/settings/webhooks",
                                            "/api/integrations", "/api/callbacks",   "/admin/webhooks"};

  for (const auto& path : webhook_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 || resp.status_code == 401 || resp.status_code == 403) {
      if (resp.status_code == 200 && (resp.body.find("webhook") != std::string::npos || resp.body.find("callback") != std::string::npos ||
...
std::vector<Finding> scan_ssrf_generators(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Look for PDF/image generation endpoints
  std::vector<std::string> gen_paths = {"/api/pdf",    "/api/generate-pdf", "/api/export/pdf", "/api/screenshot",
