...
std::vector<Finding> scan_admin_takeover(const Config &, HttpClient &http,
  ...
  std::vector<std::string> admin_paths = {
      ...
      "/manage", "/management", "/portal", "/dashboard/admin",
      "/admin/dashboard", "/backend", "/cms", "/cpanel",
      "/_admin", "/admin.php", "/admin/login", "/panel",
      "/supervisor", "/staff", "/internal", "/ops"};
  ...
  for (const auto &path : admin_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 500 &&
        ...
        (resp.body.find("dashboard") != std::string::npos ||
         ...
         resp.body.find("users") != std::string::npos ||
      ...
      findings.push_back(Finding{"Admin Panel — Unauthenticated Access", "critical",
                          base + path,
...
std::vector<Finding> scan_db_exposure(const Config &, HttpClient &http,
                                       const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto &check : checks) {
    ...
    if (resp.status_code == 200 &&
        ...
        resp.body.find("Access Denied") == std::string::npos) {
      findings.push_back(Finding{"Database Exposed — " + check.db_type, "critical",
                          base + check.path,
                          check.db_type + " management interface publicly accessible without auth. "
                          "Full database read/write possible.",
                          "", check.path,
                          "Indicator: '" + check.indicator + "' found in response"});
      return findings;
    }
  }
  return findings;
...
std::vector<Finding> scan_rce_cves(const Config &, HttpClient &http,
                                    const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (actuator.status_code == 200 &&
      actuator.body.find("spring") != std::string::npos) {
    // Spring app with exposed actuator — check for specific RCE indicators
    auto health = http.get(base + "/actuator/health");
    if (health.status_code == 200) {
      findings.push_back(Finding{"Spring Boot Actuator — Potential RCE", "critical",
                          base + "/actuator/env",
                          ...
                          "", "/actuator/env",
                          "Spring application properties visible"});
    }
  }

  // Apache Struts (CVE-2017-5638 style) — OGNL injection via Content-Type
  auto struts = http.get(base + "/", {{"Content-Type",
      "%{(#_='multipart/form-data').(#dm=@ognl.OgnlContext@DEFAULT_MEMBER_ACCESS)}"
  }});
  if (struts.status_code == 200 &&
      struts.body.find("ognl") != std::string::npos) {
  ...
  if (jmx.status_code == 200 &&
      jmx.body.find("JBoss") != std::string::npos &&
      jmx.body.find("login") == std::string::npos) {
    findings.push_back(Finding{"JBoss JMX Console — Unauthenticated RCE", "critical",
                        base + "/jmx-console/",
  ...
  if (jenkins.status_code == 200 &&
      jenkins.body.find("Groovy") != std::string::npos &&
      jenkins.body.find("login") == std::string::npos) {
    findings.push_back(Finding{"Jenkins Script Console — Unauthenticated RCE", "critical",
                        base + "/script",
                        "Jenkins Groovy script console accessible without auth. "
                        "Execute arbitrary code on the server.",
                        "", "/script",
                        "Groovy console found without login requirement"});
  }
...
std::vector<Finding> scan_mass_assignment(const Config &, HttpClient &http,
                                           const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::vector<std::string> reg_paths = {
      ...
      "/api/users", "/api/v1/users", "/api/account"};

  for (const auto &path : reg_paths) {
    // Try adding admin/role fields to registration
    std::string payload = R"({"email":"test@test.com","password":"Test1234!","role":"admin","is_admin":true,"admin":true,"type":"administrator"})";
    ...
    if (resp.status_code == 200 || resp.status_code == 201) {
      ...
      if (resp.body.find("\"admin\"") != std::string::npos ||
          resp.body.find("\"administrator\"") != std::string::npos ||
          resp.body.find("\"role\":\"admin\"") != std::string::npos) {
        findings.push_back(Finding{"Mass Assignment → Admin Privilege Escalation", "critical",
                            base + path,
...
std::vector<Finding> scan_auth_bypass_tokens(const Config &, HttpClient &http,
                                              const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::vector<std::string> protected_paths = {
      "/api/me", "/api/user", "/api/v1/me", "/api/profile",
      "/api/admin", "/api/v1/admin", "/api/account",
      "/api/users", "/api/v1/users/1", "/dashboard/api"};
  ...
  for (const auto &path : protected_paths) {
    ...
    for (const auto &bp : bypasses) {
      ...
      if (resp.status_code == 200 && resp.body.size() > 100 &&
        ...
        findings.push_back(Finding{"Authentication Bypass — " + bp.name, "critical",
                            base + path,
...
std::vector<Finding> scan_pii_exposure(const Config &, HttpClient &http,
                                        const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::vector<std::string> data_paths = {
      "/api/users", "/api/v1/users", "/api/customers",
      "/api/v1/customers", "/api/members", "/api/staff",
      "/api/employees", "/api/accounts", "/graphql"};

  std::vector<std::string> pii_indicators = {
      "\"email\"", "\"phone\"", "\"ssn\"", "\"password\"",
      "\"credit_card\"", "\"address\"", "\"date_of_birth\"",
      "\"social_security\"", "\"bank_account\"", "\"passport\""};
  ...
  for (const auto &path : data_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 200 &&
      ...
      if (pii_count >= 2) {
        findings.push_back(Finding{"PII Exposure — Public API", "critical",
                            base + path,
                            "Public API endpoint returns user PII without authentication. "
                            + std::to_string(pii_count) + " PII fields detected.",
                            "", path,
                            "Response contains: " + resp.body.substr(0, 300)});
        return findings;
      }
    }
  }
...
std::vector<Finding> scan_sqli_confirmed(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
    ...
    for (; it != end; ++it) {
      ...
      for (const auto &[payload, indicator] : error_payloads) {
        ...
        if (resp.status_code == 200 || resp.status_code == 500) {
          ...
          if (resp.body.find("You have an error in your SQL syntax") != std::string::npos ||
              resp.body.find("mysql_fetch") != std::string::npos ||
              resp.body.find("ORA-01756") != std::string::npos ||
              resp.body.find("SQLSTATE") != std::string::npos ||
              resp.body.find("Microsoft OLE DB") != std::string::npos ||
              resp.body.find("Unclosed quotation mark") != std::string::npos ||
              resp.body.find("pg_query") != std::string::npos ||
              resp.body.find("SQLite3::") != std::string::npos) {
            findings.push_back(Finding{"SQL Injection — Error Based (Confirmed)", "critical",
                                inject_url + payload,
                                "Database error triggered by SQL metacharacters. "
                                "Confirmed injectable parameter.",
                                param, payload,
                                resp.body.substr(resp.body.find("SQL") != std::string::npos ?
                                    resp.body.find("SQL") : 0, 200)});
            return findings;
          }
        }
      }
      ...
      if (elapsed > 3800) {
        ...
        if (elapsed2 > 1800 && elapsed2 < elapsed) {
          findings.push_back(Finding{"SQL Injection — Time-Based Blind (Confirmed)", "critical",
                              inject_url + val + "' AND SLEEP(4)--",
                              "Confirmed: SLEEP(4)=" + std::to_string(elapsed) + "ms, "
                              "SLEEP(2)=" + std::to_string(elapsed2) + "ms. "
...
std::vector<Finding> scan_password_reset_takeover(const Config &, HttpClient &http,
                                                   const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::vector<std::string> reset_paths = {
      ...
      "/api/v1/password/reset", "/auth/forgot"};
  ...
  for (const auto &path : reset_paths) {
    ...
    if (resp.status_code == 200 &&
        (resp.body.find("success") != std::string::npos ||
         resp.body.find("sent") != std::string::npos ||
         resp.body.find("email") != std::string::npos) &&
        resp.body.find("error") == std::string::npos &&
        resp.body.find("Access Denied") == std::string::npos) {
      // If the reset email is sent with our evil host in the link = ATO
      findings.push_back(Finding{"Password Reset Poisoning → Account Takeover", "critical",
                          base + path,
                          ...
                          "Host", "evil.com",
                          "Reset accepted with Host: evil.com"});
      return findings;
    }
  }
  return findings;
