...
std::vector<Finding> scan_grpc_reflection(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  std::string base = base_url_from(crawl.urls[0]);
  const std::vector<std::string> grpc_paths = {"/grpc.reflection.v1alpha.ServerReflection/ServerReflectionInfo",
                                               "/grpc.reflection.v1.ServerReflection/ServerReflectionInfo", "/grpc.health.v1.Health/Check"};
    ...
    if (has_grpc_header || resp.status_code == 200) {
      ...
      if (grpc_status == "0" || grpc_status == "12" || resp.status_code == 200) {
        findings.push_back({"gRPC Reflection Exposed", "high", base + path,
                            "gRPC reflection service accessible — all service/method names leaked", "", path,
  ...
  auto resp = http.post(base + "/grpc.reflection.v1alpha.ServerReflection/ServerReflectionInfo", "", "application/grpc-web+proto");
  if (resp.status_code == 200 && resp.body.size() > 0) {
    findings.push_back({"gRPC-Web Reflection", "medium", base, "gRPC-Web reflection endpoint responds — service enumeration possible", "",
                        "grpc-web+proto", ""});
  }
  return findings;
}
...
std::vector<Finding> scan_graphql_subscription_abuse(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  std::string base = base_url_from(crawl.urls[0]);
  const std::vector<std::string> gql_paths = {"/graphql", "/api/graphql", "/v1/graphql", "/subscriptions", "/graphql/subscriptions"};

  for (const auto& path : gql_paths) {
    // Test if subscriptions are enabled by sending introspection for subscription type
    std::string introspect_query = R"({"query":"{ __schema { subscriptionType { name fields { name } } } }"})";
    auto resp = http.post(base + path, introspect_query, "application/json");

    if (resp.status_code == 200 && resp.body.find("subscriptionType") != std::string::npos && resp.body.find("null") == std::string::npos) {
      findings.push_back({"GraphQL Subscription Exposed", "high", base + path,
                          ...
                          "", "subscriptionType introspection", resp.body.substr(0, 200)});

      // Attempt unauthenticated subscription connection
      auto ws_resp = http.get(base + path, {{"Upgrade", "websocket"}, {"Connection", "Upgrade"}, {"Sec-WebSocket-Protocol", "graphql-ws"}});
      if (ws_resp.status_code == 101 || ws_resp.status_code == 200) {
        findings.push_back({"GraphQL WebSocket Hijack", "critical", base + path,
                            "Unauthenticated WebSocket upgrade accepted for GraphQL subscriptions", "", "graphql-ws protocol", ""});
      }
      break;
    }
  }
...
std::vector<Finding> scan_rest_mass_assignment(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  std::string base = base_url_from(crawl.urls[0]);
  const std::vector<std::string> api_paths = {"/api/user", "/api/users/me", "/api/profile", "/api/v1/user", "/api/account"};
  const std::vector<std::string> dangerous_fields = {"\"role\":\"admin\"",      "\"is_admin\":true", "\"isAdmin\":true",
  ...
  for (const auto& path : api_paths) {
    ...
    for (const auto& field : dangerous_fields) {
      ...
      auto patch_resp = http.post(base + path, payload, "application/json", {{"X-HTTP-Method-Override", "PATCH"}});

      if (patch_resp.status_code == 200 || patch_resp.status_code == 204) {
        // Verify field was accepted (no error about unknown fields)
        if (patch_resp.body.find("error") == std::string::npos && patch_resp.body.find("unknown") == std::string::npos &&
...
std::vector<Finding> scan_soap_injection(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  std::string base = base_url_from(crawl.urls[0]);
  const std::vector<std::string> wsdl_paths = {"/ws?wsdl",       "/service?wsdl",    "/soap?wsdl", "/api/soap?wsdl",
                                               "/services?wsdl", "/webservice?wsdl", "/?wsdl"};

  for (const auto& path : wsdl_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && (resp.body.find("<wsdl:") != std::string::npos || resp.body.find("<definitions") != std::string::npos)) {
      findings.push_back({"WSDL Exposed", "medium", base + path, "SOAP WSDL publicly accessible — all operations and types disclosed", "",
      ...
      if (xxe_resp.status_code == 200 && xxe_resp.body.find("<!ENTITY") == std::string::npos && xxe_resp.body.size() > 100) {
        ...
        if (xxe_resp.body.find("fault") == std::string::npos) {
          findings.push_back({"SOAP XXE Injection", "critical", base + path,
                              ...
                              "", "DOCTYPE entity injection", xxe_resp.body.substr(0, 200)});
        }
      }
      break;
    }
...
std::vector<Finding> scan_jsonrpc_enumeration(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  std::string base = base_url_from(crawl.urls[0]);
  const std::vector<std::string> rpc_paths = {"/jsonrpc", "/rpc", "/api/rpc", "/json-rpc", "/api/jsonrpc"};
  const std::vector<std::string> probe_methods = {"system.listMethods", "rpc.discover", "system.methodHelp", "system.methodSignature",
                                                  "debug.trace",        "admin.peers",  "eth_accounts",      "personal_listAccounts"};

  for (const auto& path : rpc_paths) {
    // First test if JSON-RPC endpoint exists
    std::string discovery = R"({"jsonrpc":"2.0","method":"system.listMethods","id":1})";
    ...
    if (resp.body.find("result") != std::string::npos && resp.body.find("error") == std::string::npos) {
      findings.push_back({"JSON-RPC Method Listing", "high", base + path,
                          "JSON-RPC system.listMethods accessible — full API surface exposed", "", "system.listMethods",
    ...
    for (const auto& method : probe_methods) {
      std::string payload = R"({"jsonrpc":"2.0","method":")" + method + R"(","params":[],"id":1})";
      auto m_resp = http.post(base + path, payload, "application/json");
      if (m_resp.status_code == 200 && m_resp.body.find("result") != std::string::npos &&
          m_resp.body.find("Method not found") == std::string::npos) {
        findings.push_back({"JSON-RPC Sensitive Method", "high", base + path, "Sensitive JSON-RPC method accessible: " + method, "", method,
...
std::vector<Finding> scan_websocket_tampering(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  ...
  for (const auto& ws_url : ws_urls) {
    ...
    auto resp =
        http.get(http_url,
                 {{"Upgrade", "websocket"}, {"Connection", "Upgrade"}, {"Sec-WebSocket-Version", "13"}, {"Sec-WebSocket-Key", "dGVzdA=="}});
    if (resp.status_code == 101) {
      findings.push_back({"WebSocket Auth Bypass", "high", ws_url, "WebSocket endpoint accepts upgrade without authentication", "",
                          "No-auth upgrade accepted", "HTTP 101 Switching Protocols"});
      break;
    }
...
std::vector<Finding> scan_sse_data_leak(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  std::string base = base_url_from(crawl.urls[0]);
  const std::vector<std::string> sse_paths = {"/events",  "/stream",   "/api/events",   "/api/stream",           "/sse",
                                              "/api/sse", "/realtime", "/api/realtime", "/notifications/stream", "/updates"};
  const std::vector<std::string> sensitive_patterns = {"password", "token",   "secret", "private", "internal",
  ...
  for (const auto& path : sse_paths) {
    ...
    bool has_sse_format = resp.body.find("data:") != std::string::npos || resp.body.find("event:") != std::string::npos;
    ...
    if (is_sse || has_sse_format) {
      ...
      for (const auto& pattern : sensitive_patterns) {
        if (resp.body.find(pattern) != std::string::npos) {
          findings.push_back({"SSE Sensitive Data Leak", "high", base + path,
                              "Server-Sent Events stream exposes sensitive data: " + pattern, "", "text/event-stream",
                              resp.body.substr(0, 200)});
          return findings;
        }
      }
      ...
      findings.push_back({"SSE Endpoint Open", "low", base + path, "Server-Sent Events endpoint accessible without authentication", "",
                          "text/event-stream", ""});
      return findings;
    }
  }
...
std::vector<Finding> scan_odata_injection(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;

  std::string base = base_url_from(crawl.urls[0]);
  const std::vector<std::string> odata_paths = {"/odata", "/api/odata", "/v1/odata", "/api/v1", "/api/v2", "/api/entities", "/api/data"};
  ...
  for (const auto& path : odata_paths) {
    ...
    bool is_odata = (meta_resp.status_code == 200 &&
                     (meta_resp.body.find("edmx") != std::string::npos || meta_resp.body.find("EntityType") != std::string::npos));

    if (!is_odata) {
      // Try basic OData query to confirm
      auto probe = http.get(base + path + "?$top=1");
      if (probe.status_code != 200 || probe.body.find("value") == std::string::npos) continue;
    }

    // Test $filter injection for auth bypass
    const std::vector<std::string> filter_payloads = {"?$filter=1 eq 1", "?$filter=IsAdmin eq true", "?$filter=Role eq 'admin'",
    ...
    for (const auto& filter : filter_payloads) {
      ...
      if (resp.status_code == 200 && resp.body.find("error") == std::string::npos && resp.body.find("value") != std::string::npos &&
          resp.body.size() > 50) {
