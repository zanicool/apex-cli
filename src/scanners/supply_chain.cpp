...
std::vector<Finding> scan_self_hosted_tools(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& t : tools) {
    ...
    if (resp.status_code == 200 && resp.body.find(t.fingerprint) != std::string::npos) {
      findings.push_back({"Supply Chain: Self-Hosted", t.severity, url, std::string(t.name) + " exposed on subdomain", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_cloud_storage_exposure(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  size_t start = base.find("://") + 3;
  ...
  // S3 bucket patterns.
  const std::vector<std::string> s3_suffixes = {"",      "-backup", "-assets", "-uploads", "-data",     "-dev", "-staging",
  ...
  for (const auto& suffix : s3_suffixes) {
    ...
    auto resp = http.get(url);
    if (resp.status_code == 200 && resp.body.find("<EnumerationResults") != std::string::npos) {
      findings.push_back({"Supply Chain: Azure Blob Public", "critical", url, "Public Azure Blob container: " + container, "", "", ""});
    }
  }
  ...
  if (gcs.status_code == 200 && gcs.body.find("<ListBucket") != std::string::npos) {
    findings.push_back(
        {"Supply Chain: GCS Public", "critical", "https://storage.googleapis.com/" + org + "/", "Public GCS bucket: " + org, "", "", ""});
  }
  return findings;
}

/// Detect third-party integrations leaking data (analytics, error tracking).
std::vector<Finding> scan_integration_leaks(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  for (const auto& i : integrations) {
    if (resp.body.find(i.pattern) != std::string::npos) {
      findings.push_back({"Supply Chain: Integration", "info", crawl.urls[0], std::string(i.name) + " — " + i.risk, "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_management_panels(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& p : panels) {
    ...
    auto resp = http.get(url);
    if (resp.status_code == 200 && resp.body.find(p.fingerprint) != std::string::npos) {
      findings.push_back({"Supply Chain: Management Panel", "high", url, std::string(p.name) + " exposed", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_sso_misconfig(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  bool has_sso = resp.body.find("oauth") != std::string::npos || resp.body.find("saml") != std::string::npos ||
                 resp.body.find("Sign in with") != std::string::npos || resp.body.find("Microsoft") != std::string::npos;
  bool has_password = resp.body.find("password") != std::string::npos && resp.body.find("type=\"password\"") != std::string::npos;

  if (has_sso && has_password) {
    findings.push_back({"Supply Chain: SSO Bypass", "medium", base, "SSO available but password login not disabled", "", "", ""});
  }
  return findings;
}

/// Detect exposed API keys/tokens from third-party services in page source.
std::vector<Finding> scan_exposed_api_keys(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  for (const auto& url : crawl.urls) {
    ...
    const KeyPattern patterns[] = {
        {"AIza", "Google API Key"},          {"AKIA", "AWS Access Key"},
        {"sk_live_", "Stripe Live Key"},     {"pk_live_", "Stripe Publishable Key"},
        {"xoxb-", "Slack Bot Token"},        {"ghp_", "GitHub Personal Token"},
        {"glpat-", "GitLab Personal Token"}, {"sq0csp-", "Square OAuth Secret"},
        {"SG.", "SendGrid API Key"},
    };
    ...
    for (const auto& kp : patterns) {
      if (resp.body.find(kp.pattern) != std::string::npos) {
        findings.push_back(
            {"Supply Chain: API Key Exposed", "critical", url, std::string(kp.service) + " key found in source", "", kp.pattern, ""});
      }
    }
