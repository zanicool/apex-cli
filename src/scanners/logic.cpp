...
std::vector<Finding> scan_race_condition(const Config &, HttpClient &http,
                                         const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> race_paths = {
      "/api/transfer", "/api/redeem", "/api/coupon", "/api/withdraw"};
  ...
  for (const auto &path : race_paths) {
    ...
    for (int i = 0; i < 5; ++i) {
      futures.push_back(std::async(std::launch::async, [&]() {
        return http.post(base + path, "{\"amount\":1}", "application/json");
      }));
    }
    int success = 0;
    for (auto &f : futures) {
    ...
    if (success > 1) {
      findings.push_back({"Race Condition", "high", base + path,
                          "Multiple concurrent requests succeeded (" +
                              std::to_string(success) + "/5)",
                          "", "", ""});
    }
  }
  return findings;
...
std::vector<Finding> scan_price_manipulation(const Config &, HttpClient &http,
  ...
  for (const auto &url : crawl.urls) {
    for (const auto &p : crawl.params) {
      ...
      if (p.name.find("price") == std::string::npos &&
          p.name.find("amount") == std::string::npos &&
          p.name.find("cost") == std::string::npos &&
          p.name.find("total") == std::string::npos)
        continue;
      ...
      if (resp.status_code == 200 &&
        ...
        findings.push_back({"Price Manipulation", "high", url,
                            "Price parameter accepted modified value",
                            p.name, "0.01", ""});
      }
    }
  }
...
std::vector<Finding> scan_forced_browsing(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> paths = {
      "/admin", "/admin/", "/debug", "/debug/", "/console",
      "/phpmyadmin", "/adminer", "/wp-admin", "/manager",
      "/actuator/env", "/elmah.axd", "/_profiler"};
...
std::vector<Finding> scan_idor_uuid(const Config &, HttpClient &http,
                                    const CrawlResult &crawl) {
  std::vector<Finding> findings;
...
std::vector<Finding> scan_payment_bypass(const Config &, HttpClient &http,
  ...
  const std::vector<std::string> post_payment = {
      "/order/confirm", "/checkout/success", "/payment/complete",
      "/api/order/complete", "/thank-you"};
  ...
  for (const auto &path : post_payment) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 100 &&
      ...
      findings.push_back({"Payment Bypass", "high", base + path,
                          "Post-payment page accessible without payment",
                          "", "", ""});
    }
  }
  return findings;
...
std::vector<Scanner> register_logic_scanners() {
  return {
      {"Race Condition", scan_race_condition},
      {"Price Manipulation", scan_price_manipulation},
      {"Mass Assignment", scan_mass_assignment},
      {"Forced Browsing", scan_forced_browsing},
      {"IDOR (UUID)", scan_idor_uuid},
