...
std::vector<Finding> scan_error_disclosure(const Config &, HttpClient &http,
  ...
  for (const auto &[url, trigger_type] : error_triggers) {
    ...
    if (resp.status_code >= 400 && resp.body.size() > 100) {
      ...
      if (resp.body.find("Traceback") != std::string::npos ||
          resp.body.find("at ") != std::string::npos ||
          resp.body.find("Exception") != std::string::npos ||
          resp.body.find("stack trace") != std::string::npos) {
      ...
      // Check for DB errors
      if (resp.body.find("SQL") != std::string::npos ||
          resp.body.find("mysql") != std::string::npos ||
          resp.body.find("postgres") != std::string::npos ||
      ...
      if (std::regex_search(resp.body, m, path_re)) {
        findings.push_back(Finding{"Internal Path Disclosed", "low", url,
                            "Server error reveals internal file system path: " + m[0].str(),
                            "", "", ""});
      }
...
std::vector<Finding> scan_internal_ip_leak(const Config &, HttpClient &http,
                                            const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::regex internal_ip_re(R"x(\b(10\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.(1[6-9]|2\d|3[01])\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3})\b)x");
  ...
  if (it != end) {
    findings.push_back(Finding{"Internal IP Leak — Body", "low", base,
                        "Response body contains internal IP: " + (*it).str(),
                        "", (*it).str(), ""});
  }
...
std::vector<Finding> scan_security_headers(const Config &, HttpClient &http,
                                            const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto &check : checks) {
    ...
    for (const auto &[key, val] : resp.headers) {
      ...
      if (lower == check_lower) { found = true; break; }
    }
    if (!found) {
      findings.push_back(Finding{"Missing " + check.header, check.severity, base,
                          check.missing_msg, "", "", ""});
    }
  }
  ...
  for (const auto &[key, val] : resp.headers) {
    ...
    if (lower == "content-security-policy") {
      if (val.find("unsafe-inline") != std::string::npos &&
          val.find("unsafe-eval") != std::string::npos) {
      ...
      if (val.find("*") != std::string::npos) {
        findings.push_back(Finding{"CSP — Wildcard Source", "medium", base,
                            "CSP contains wildcard (*) source. Allows loading from any domain.",
                            "CSP", val.substr(0, 200), ""});
      }
      break;
    }
...
std::vector<Finding> scan_tech_fingerprint(const Config &, HttpClient &http,
                                            const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (powered != resp.headers.end()) {
    findings.push_back(Finding{"X-Powered-By Disclosed", "low", base,
                        "X-Powered-By reveals: " + powered->second,
