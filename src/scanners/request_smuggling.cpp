...
std::vector<Finding> scan_clte_smuggling(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  auto baseline_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start).count();

  // CL.TE probe: Content-Length shorter than chunked body
  std::string smuggle_body = "0\r\n\r\n";
  start = std::chrono::steady_clock::now();
  http.post(base + "/", smuggle_body, "application/x-www-form-urlencoded", {{"Content-Length", "4"}, {"Transfer-Encoding", "chunked"}});
  auto probe_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start).count();
...
std::vector<Finding> scan_tecl_smuggling(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  http.post(base + "/", body, "application/x-www-form-urlencoded",
            {{"Transfer-Encoding", "chunked"}, {"Content-Length", std::to_string(body.size() + 100)}});
  auto probe_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start).count();
...
std::vector<Finding> scan_tete_smuggling(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& [header, value] : te_variants) {
    auto resp = http.post(base + "/", body, "application/x-www-form-urlencoded", {{header, value}, {"Content-Length", "5"}});
...
std::vector<Finding> scan_h2_smuggling(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (server != resp.headers.end()) {
    if (server->second.find("nginx") != std::string::npos || server->second.find("cloudflare") != std::string::npos) {
      has_h2 = true;
    }
  }

  if (has_h2) {
    auto probe = http.post(base + "/", "x", "text/plain", {{"Content-Length", "0"}});
