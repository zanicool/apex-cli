...
std::vector<Finding> scan_wp_config_backup(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> backup_paths = {
      "/wp-config.php.bak", "/wp-config.php.old", "/wp-config.php.save", "/wp-config.php.swp", "/wp-config.php~", "/wp-config.bak",
  };

  for (const auto& path : backup_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && resp.body.find("<?php") != std::string::npos && resp.body.find("DB_PASSWORD") != std::string::npos &&
...
std::vector<Finding> scan_wp_xmlrpc_abuse(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (resp.status_code == 200 && resp.body.find("<methodResponse>") != std::string::npos &&
    ...
    auto ping = http.post(base + "/xmlrpc.php",
                          ...
                          "text/xml");
    if (ping.status_code == 200 && ping.body.find("<fault>") == std::string::npos) {
...
std::vector<Finding> scan_wp_user_enum(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (int i = 1; i <= 5; ++i) {
    ...
    if ((resp.status_code == 301 || resp.status_code == 302) && resp.headers.count("location") &&
    ...
    if (resp.status_code == 200 && resp.body.find("author-") != std::string::npos && resp.body.find("posts by") != std::string::npos) {
      findings.push_back({"WP User Enumeration", "medium", base + "/?author=" + std::to_string(i),
...
std::vector<Finding> scan_wp_rest_user_leak(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  auto resp = http.get(base + "/wp-json/wp/v2/users");
  if (resp.status_code == 200 && resp.body.find("\"slug\"") != std::string::npos && resp.body.find("\"name\"") != std::string::npos &&
      resp.body.find("\"id\"") != std::string::npos && resp.body.front() == '[') {
    findings.push_back({"WP REST API User Leak", "medium", base + "/wp-json/wp/v2/users",
...
std::vector<Finding> scan_wp_plugin_vulns(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // RevSlider arbitrary file download (CVE-2014-9734)
  auto revslider = http.get(base + "/wp-admin/admin-ajax.php?action=revslider_show_image&img=../wp-config.php");
  if (revslider.status_code == 200 && revslider.body.find("<?php") != std::string::npos &&
      revslider.body.find("DB_PASSWORD") != std::string::npos) {
    findings.push_back({"WP RevSlider File Download", "critical", base + "/wp-admin/admin-ajax.php?action=revslider_show_image",
  ...
  if (cf7_readme.status_code == 200 && cf7_readme.body.find("Contact Form 7") != std::string::npos &&
    ...
    if (std::regex_search(cf7_readme.body, match, ver_re)) {
      ...
      if (version < "5.3.2") {
        findings.push_back({"WP CF7 Upload Vuln", "high", base + "/wp-content/plugins/contact-form-7/readme.txt",
                            "Contact Form 7 version " + version +
                                " vulnerable to "
...
std::vector<Finding> scan_wp_cron_dos(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (resp.status_code == 200 && resp.body.size() < 100 && resp.body.find("404") == std::string::npos &&
    ...
    if (verify.status_code == 200 && verify.body.find("wp-login") != std::string::npos) {
...
std::vector<Scanner> register_cms_specific_scanners() {
  return {
      {"WP Config Backup", scan_wp_config_backup},   {"WP XMLRPC Abuse", scan_wp_xmlrpc_abuse}, {"WP User Enum", scan_wp_user_enum},
      {"WP REST User Leak", scan_wp_rest_user_leak}, {"WP Plugin Vulns", scan_wp_plugin_vulns}, {"WP Cron DoS", scan_wp_cron_dos},
  };
}
