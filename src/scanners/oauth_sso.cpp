...
std::vector<Finding> scan_oauth_discovery(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  if (oidc.status_code == 200 && oidc.body.find("authorization_endpoint") != std::string::npos) {
    findings.push_back(Finding{"OpenID Connect Configuration Exposed", "info", base + "/.well-known/openid-configuration",
    ...
    if (oidc.body.find("\"token\"") != std::string::npos && oidc.body.find("\"id_token\"") != std::string::npos) {
  ...
  std::vector<std::string> auth_paths = {"/oauth/authorize", "/authorize", "/oauth2/authorize", "/api/oauth/authorize",
                                         "/connect/authorize"};
...
std::vector<Finding> scan_oauth_redirect(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::vector<std::pair<std::string, std::string>> bypasses = {
      {original_redirect + ".evil.com", "subdomain append"},          {original_redirect + "%40evil.com", "@ symbol confusion"},
      {original_redirect + "/../evil.com", "path traversal"},         {original_redirect + "%2f%2fevil.com", "double-encoded slash"},
      {"https://evil.com%23" + original_redirect, "fragment bypass"}, {original_redirect + "?next=https://evil.com", "open redirect param"},
  };
  ...
  for (const auto& [payload, technique] : bypasses) {
    ...
    findings.push_back(Finding{"OAuth — redirect_uri Bypass Vector", "info", base,
                               "Test redirect_uri manipulation via " + technique + ": " + payload, "redirect_uri", payload, ""});
  }
...
std::vector<Finding> scan_oauth_state(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
...
std::vector<Finding> scan_token_leakage(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  if (resp.body.find("access_token") != std::string::npos || resp.body.find("id_token") != std::string::npos) {
