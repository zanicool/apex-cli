...
std::vector<Finding> scan_tls(const Config &cfg, HttpClient &http,
  ...
  std::string cert_info = run_cmd("echo | openssl s_client -connect " + domain +
  ...
  struct Proto { const char *name; const char *flag; const char *severity; };
  ...
  for (const auto &p : weak_protos) {
    ...
    if (result.find("CONNECTED") != std::string::npos &&
      ...
      findings.push_back({"Weak TLS Protocol: " + std::string(p.name), p.severity,
                          domain, std::string(p.name) + " still enabled — should be disabled",
                          "", "", ""});
    }
  }
  ...
  if (!crawl.urls.empty()) {
    ...
    if (resp.headers.find("Strict-Transport-Security") == resp.headers.end()) {
      findings.push_back({"Missing HSTS", "medium", domain,
...
std::vector<Finding> scan_email_security(const Config &cfg, HttpClient &,
  ...
  if (spf.empty()) {
    findings.push_back({"Missing SPF Record", "medium", domain,
                        "No SPF record — email spoofing possible", "", "", ""});
  } else {
    if (spf.find("+all") != std::string::npos) {
      findings.push_back({"Weak SPF: +all", "high", domain,
                          "SPF uses +all — allows any server to send email", "", "", ""});
    } else if (spf.find("~all") != std::string::npos) {
      findings.push_back({"SPF Softfail", "low", domain,
                          "SPF uses ~all (softfail) — consider -all (hardfail)", "", "", ""});
    } else {
  ...
  if (dmarc.empty() || dmarc.find("v=DMARC") == std::string::npos) {
    findings.push_back({"Missing DMARC Record", "medium", domain,
                        "No DMARC record — no email authentication policy", "", "", ""});
  } else {
    if (dmarc.find("p=none") != std::string::npos) {
      findings.push_back({"DMARC Policy: none", "low", domain,
  ...
  for (const auto &sel : selectors) {
    ...
    if (dkim.find("v=DKIM") != std::string::npos || dkim.find("p=") != std::string::npos) {
      dkim_found = true;
      findings.push_back({"DKIM Record", "info", domain,
                          "DKIM found at selector: " + sel, "", "", ""});
      break;
    }
  }
  if (!dkim_found) {
    findings.push_back({"No DKIM Found", "low", domain,
  ...
  if (mta_sts.empty() || mta_sts.find("v=STSv1") == std::string::npos) {
    findings.push_back({"Missing MTA-STS", "info", domain,
                        "No MTA-STS — email transport not enforcing TLS", "", "", ""});
  }
...
std::vector<Finding> scan_dns_security(const Config &cfg, HttpClient &,
  ...
  if (rrsig.empty()) {
    findings.push_back({"No DNSSEC", "low", domain,
                        "DNSSEC not enabled — DNS responses not authenticated", "", "", ""});
  } else {
  ...
  if (caa.empty()) {
    findings.push_back({"Missing CAA Record", "low", domain,
  ...
  if (!wildcard.empty() && wildcard.find("NXDOMAIN") == std::string::npos &&
      wildcard.size() > 5) {
    findings.push_back({"Wildcard DNS", "low", domain,
                        "Wildcard DNS record exists — may mask subdomain takeover",
                        "", "", ""});
  }
