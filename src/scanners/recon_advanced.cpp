...
std::vector<Finding> scan_internal_ip(const Config&, HttpClient& http, const CrawlResult& crawl) {
...
std::vector<Finding> scan_staging_env(const Config& cfg, HttpClient& http, const CrawlResult&) {
  std::vector<Finding> findings;
...
std::vector<Finding> scan_k8s_docker(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  for (const auto& [path, name] : k8s_paths) {
    ...
    if (resp.status_code == 200 && resp.body.size() > 20) {
      if (path == "/v2/_catalog" && resp.body.find("repositories") != std::string::npos) {
        findings.push_back({"Docker Registry Open", "critical", base + path, "Docker registry publicly accessible — images can be pulled",
                            "", "", resp.body.substr(0, 200)});
      } else if (path == "/api/v1/namespaces" && resp.body.find("items") != std::string::npos) {
        findings.push_back(
            {"Kubernetes API Exposed", "critical", base + path, "K8s API server accessible — cluster compromise possible", "", "", ""});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_cicd_leak(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& [path, name] : ci_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && resp.body.size() > 20 && resp.body.find("<html") == std::string::npos) {
      findings.push_back({"CI/CD Config Exposed: " + name, "medium", base + path, name + " configuration publicly accessible", "", "",
                          std::to_string(resp.body.size()) + " bytes"});
    }
  }
  return findings;
...
std::vector<Finding> scan_js_libraries(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  if (resp.body.find("jquery") != std::string::npos) {
    ...
    if (std::regex_search(resp.body, m, jq_re)) {
      ...
      if (ver[0] == '1' || (ver[0] == '2' && ver.size() > 2)) {
        findings.push_back({"Outdated jQuery: " + ver, "medium", crawl.urls[0], "jQuery " + ver + " has known XSS vulnerabilities", "", "",
                            "CVE-2020-11022, CVE-2020-11023"});
      }
    }
  }
  if (resp.body.find("angular") != std::string::npos && resp.body.find("angular.js") != std::string::npos) {
    findings.push_back({"AngularJS Detected", "low", crawl.urls[0], "AngularJS (1.x) is end-of-life — template injection possible", "", "",
  ...
  if (resp.body.find("lodash") != std::string::npos) {
    ...
    if (std::regex_search(resp.body, m, lodash_re)) {
      findings.push_back({"Outdated Lodash: " + m[1].str(), "low", crawl.urls[0], "Lodash " + m[1].str() + " has prototype pollution CVEs",
...
std::vector<Finding> scan_email_security(const Config& cfg, HttpClient&, const CrawlResult&) {
  ...
  if (fp) {
    ...
    std::string spf(buf);
    if (spf.empty()) {
      findings.push_back({"Missing SPF Record", "medium", cfg.target, "No SPF record — domain can be spoofed for phishing", "", "", ""});
    } else if (spf.find("+all") != std::string::npos) {
      findings.push_back(
          {"SPF +all (Permissive)", "high", cfg.target, "SPF allows any server to send email as this domain", "", "", spf.substr(0, 100)});
    }
  }
  ...
  if (fp) {
    ...
    if (dmarc.empty() || dmarc.find("v=DMARC") == std::string::npos) {
      findings.push_back({"Missing DMARC Record", "medium", cfg.target, "No DMARC policy — email spoofing not prevented", "", "", ""});
    } else if (dmarc.find("p=none") != std::string::npos) {
      findings.push_back(
          {"DMARC p=none", "low", cfg.target, "DMARC in monitor-only mode — spoofed emails still delivered", "", "", dmarc.substr(0, 100)});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_exposed_metrics(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> metric_paths = {"/metrics", "/prometheus", "/grafana", "/api/metrics", "/-/metrics", "/internal/metrics"};

  for (const auto& path : metric_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && (resp.body.find("# HELP") != std::string::npos || resp.body.find("# TYPE") != std::string::npos ||
                                    resp.body.find("process_cpu") != std::string::npos)) {
      findings.push_back({"Prometheus Metrics Exposed", "medium", base + path, "Application metrics publicly accessible — info disclosure",
                          "", "", std::to_string(resp.body.size()) + " bytes"});
      break;
    }
  }
...
std::vector<Finding> scan_hidden_admin(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> admin_paths = {"/admin.php",  "/administrator/", "/admin123",   "/cpanel",    "/controlpanel",
                                                "/adminpanel", "/backend",        "/cms",        "/portal",    "/webadmin",
                                                "/siteadmin",  "/manage",         "/supervisor", "/moderator", "/staff"};
  ...
  for (const auto& path : admin_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && resp.body.size() > 200 && resp.body != not_found.body && resp.body.find("login") != std::string::npos) {
      findings.push_back({"Hidden Admin Panel", "medium", base + path, "Admin login page found at non-standard path", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_robots_sensitive(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> sensitive_keywords = {"admin",    "backup",  "config", "secret", "private",
    ...
    for (const auto& kw : sensitive_keywords) {
      if (path.find(kw) != std::string::npos) {
        ...
        if (check.status_code == 200 && check.body.size() > 100) {
          findings.push_back(
              {"Sensitive Path in robots.txt", "low", base + path, "robots.txt reveals and path is accessible: " + path, "", "", ""});
          break;
        }
      }
    }
...
std::vector<Finding> scan_git_exposure(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  if (head.status_code == 200 && head.body.find("ref:") != std::string::npos) {
    findings.push_back({"Git Repository Exposed", "critical", base + "/.git/", "Full source code recoverable via exposed .git directory",
                        "", "", "HEAD: " + head.body.substr(0, 50)});

    // Check config for credentials
    auto config = http.get(base + "/.git/config");
    if (config.status_code == 200 &&
        (config.body.find("password") != std::string::npos || config.body.find("token") != std::string::npos)) {
      findings.push_back({"Git Config — Credentials", "critical", base + "/.git/config", "Git config contains credentials/tokens", "", "",
...
/// SVN exposure.
std::vector<Finding> scan_svn_exposure(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  auto resp = http.get(base + "/.svn/entries");
  if (resp.status_code == 200 && (resp.body.find("dir") != std::string::npos || resp.body.find("svn") != std::string::npos)) {
    findings.push_back({"SVN Repository Exposed", "high", base + "/.svn/", "SVN metadata accessible — source code disclosure", "", "",
...
std::vector<Finding> scan_env_file(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  const std::vector<std::string> env_paths = {"/.env",         "/.env.production",  "/.env.local",
                                              "/.env.staging", "/.env.development", "/.env.backup"};

  for (const auto& path : env_paths) {
    auto resp = http.get(base + path);
    if (resp.status_code == 200 && resp.body.size() > 10 && (resp.body.find("=") != std::string::npos) &&
        resp.body.find("<html") == std::string::npos) {
      // Verify it looks like an env file
      if (resp.body.find("DB_") != std::string::npos || resp.body.find("API_") != std::string::npos ||
          resp.body.find("SECRET") != std::string::npos || resp.body.find("KEY=") != std::string::npos) {
        findings.push_back({"Environment File Exposed: " + path, "critical", base + path, "Env file with secrets publicly accessible", "",
                            "", resp.body.substr(0, 100) + "..."});
        break;
      }
    }
