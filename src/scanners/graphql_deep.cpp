...
std::vector<std::string> find_graphql(HttpClient& http, const std::string& base) {
  ...
  std::vector<std::string> paths = {"/graphql", "/api/graphql", "/v1/graphql",     "/gql",           "/query", "/api/query", "/graphiql",
                                    "/altair",  "/playground",  "/api/v1/graphql", "/api/v2/graphql"};

  for (const auto& path : paths) {
    auto resp = http.post(base + path, "{\"query\":\"{__typename}\"}", "application/json");
    if (resp.status_code == 200 && (resp.body.find("__typename") != std::string::npos || resp.body.find("\"data\"") != std::string::npos ||
...
std::vector<Finding> scan_graphql_introspection(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& ep : endpoints) {
    ...
    if (resp.status_code == 200 && resp.body.find("__schema") != std::string::npos && resp.body.find("queryType") != std::string::npos) {
      // Count exposed types
      ...
      if (resp.body.find("deleteUser") != std::string::npos || resp.body.find("updateRole") != std::string::npos ||
          resp.body.find("createAdmin") != std::string::npos || resp.body.find("resetPassword") != std::string::npos) {
...
std::vector<Finding> scan_graphql_depth(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  auto endpoints = find_graphql(http, base);
  ...
  std::string deep =
      R"({"query":"{__typename a:__typename b:__typename c:__typename d:__typename e:__typename f:__typename g:__typename h:__typename i:__typename j:__typename}"})";
...
std::vector<Finding> scan_graphql_alias_bypass(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  auto endpoints = find_graphql(http, base);
  ...
  std::string batch =
      R"({"query":"{a1:__typename a2:__typename a3:__typename a4:__typename a5:__typename a6:__typename a7:__typename a8:__typename a9:__typename a10:__typename}"})";

  for (const auto& ep : endpoints) {
    auto resp = http.post(ep, batch, "application/json");
    if (resp.status_code == 200 && resp.body.find("a10") != std::string::npos) {
...
std::vector<Finding> scan_graphql_batch(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  auto endpoints = find_graphql(http, base);
  ...
  std::string batch =
      R"([{"query":"{__typename}"},{"query":"{__typename}"},{"query":"{__typename}"},{"query":"{__typename}"},{"query":"{__typename}"}])";
    ...
    if (resp.status_code == 200) {
      ...
      if (count >= 5) {
        findings.push_back(Finding{"GraphQL — Array Batch Queries Allowed", "medium", ep,
                                   "Server processes array of " + std::to_string(count) +
...
std::vector<Finding> scan_graphql_suggestions(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  auto endpoints = find_graphql(http, base);
  ...
  for (const auto& ep : endpoints) {
    ...
    if (resp.status_code == 200 || resp.status_code == 400) {
      if (resp.body.find("Did you mean") != std::string::npos || resp.body.find("suggestions") != std::string::npos ||
...
std::vector<Scanner> register_graphql_deep_scanners() {
  return {
      {"GraphQL Introspection", scan_graphql_introspection}, {"GraphQL Depth Abuse", scan_graphql_depth},
      {"GraphQL Alias Bypass", scan_graphql_alias_bypass},   {"GraphQL Batch", scan_graphql_batch},
