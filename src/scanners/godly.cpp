...
std::vector<Finding> scan_multi_step(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Try common admin paths without auth.
  const std::vector<std::string> protected_paths = {"/admin",    "/admin/",          "/dashboard", "/panel",
                                                    "/settings", "/api/admin/users", "/api/users", "/internal/config"};

  // Indicators that a page is truly protected (contains user-specific data)
  const std::vector<std::string> auth_indicators = {"\"email\"",   "\"username\"", "\"user_id\"", "\"account\"",
                                                    "\"session\"", "\"token\"",    "\"api_key\"", "admin",
                                                    "\"users\":[", "\"password\"", "\"role\""};

  // Indicators that a page is just a public landing/redirect
  const std::vector<std::string> public_indicators = {"<html",     "<!DOCTYPE",   "welcome",   "homepage", "landing",
  ...
  for (const auto& path : protected_paths) {
    ...
    if (resp.body.find("login") != std::string::npos || resp.body.find("sign in") != std::string::npos ||
    ...
    bool looks_public = contains_any(resp.body, public_indicators) && !has_sensitive;

    if (has_sensitive && !looks_public) {
      findings.push_back({"Auth Bypass", "critical", base + path, "Admin/protected resource with sensitive data accessible without auth",
...
std::vector<Finding> scan_content_discovery(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> wordlist = {"/backup",          "/backup.zip",
                                             "/db.sql",          "/.env.bak",
                                             "/config.yml",      "/wp-config.php.bak",
    ...
    if (resp.status_code == 200 && resp.body.size() > 10) {
      ...
      if (similarity(resp.body.substr(0, 200), fp_404) < 0.8) {
        findings.push_back({"Content Discovery", "info", base + path,
                            "Hidden content found (" + std::to_string(resp.body.size()) + " bytes)", "", "", ""});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_permission_boundary(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  // Try accessing user-specific endpoints with different IDs.
  const std::vector<std::string> endpoints = {"/api/user/1/profile", "/api/user/2/profile", "/api/account/1",
                                              "/api/account/2",      "/api/order/1",        "/api/order/2"};

  for (size_t i = 0; i + 1 < endpoints.size(); i += 2) {
    auto r1 = http.get(base + endpoints[i]);
    auto r2 = http.get(base + endpoints[i + 1]);
    if (r1.status_code == 200 && r2.status_code == 200 && r1.body != r2.body && r1.body.size() > 50) {
      findings.push_back({"Permission Boundary", "high", base + endpoints[i], "Can access other users' data", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_reflection_map(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
      ...
      if (resp.body.find(canary) != std::string::npos) {
        ...
        if (pos > 5) {
          ...
          if (before.find("\"") != std::string::npos)
            ctx = "attribute";
          else if (before.find("'") != std::string::npos)
            ctx = "js_string";
          else if (before.find(">") != std::string::npos)
            ctx = "html_tag";
        }
        findings.push_back({"Reflection", "info", url, "Input reflected in " + ctx + " context", param, canary, ""});
      }
    }
  }
  return findings;
...
std::vector<Finding> scan_response_diff(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  for (const auto& url : crawl.urls) {
    ...
    for (const auto& [base, param] : targets) {
      ...
      if (r1.status_code == r2.status_code && !r1.body.empty() && !r2.body.empty()) {
        double sim = similarity(r1.body, r2.body);
        if (sim > 0.3 && sim < 0.9) {
          findings.push_back({"Response Diff", "low", url,
                              "Different responses for admin/guest (sim=" + std::to_string(sim).substr(0, 4) + ")", param, "", ""});
        }
      }
    }
  }
