...
std::vector<Finding> scan_file_fingerprint(const Config&, HttpClient& http, const CrawlResult& crawl) {
  ...
  for (const auto& h : hints) {
    ...
    if (h.match[0] == '\0' || resp.body.find(h.match) != std::string::npos) {
      ...
      findings.push_back({"Version Fingerprint", "info", base + h.path,
                          std::string(h.product) + " " + h.version + " (detected via file content)", "", "", ""});
    }
  }
  return findings;
}
...
std::vector<Finding> scan_etag_fingerprint(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::regex apache_etag(R"re("([0-9a-f]+)-([0-9a-f]+)-([0-9a-f]+)")re");
  if (std::regex_match(etag, apache_etag)) {
    findings.push_back({"ETag Fingerprint (Apache)", "low", base,
                        "Apache inode-based ETag: " + etag + " — reveals file metadata, aids cache poisoning", "", "", ""});
  }

  // Nginx weak ETag: W/"timestamp-size".
  if (etag.find("W/\"") == 0) {
    findings.push_back({"ETag Fingerprint (Weak)", "info", base, "Weak ETag detected: " + etag, "", "", ""});
  }
...
std::vector<Finding> scan_error_fingerprint(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);

  auto resp = http.get(base + "/apex_nonexistent_" + std::to_string(time(nullptr)));
  ...
  const ErrorSig sigs[] = {
      {"Apache/", "Apache"},       {"nginx/", "nginx"},     {"Microsoft-IIS/", "IIS"},      {"LiteSpeed", "LiteSpeed"},
      {"openresty/", "OpenResty"}, {"Caddy", "Caddy"},      {"Tomcat/", "Apache Tomcat"},   {"Jetty(", "Eclipse Jetty"},
      {"GlassFish", "GlassFish"},  {"WildFly/", "WildFly"}, {"Kestrel", "ASP.NET Kestrel"},
  };
  ...
  for (const auto& sig : sigs) {
    ...
    if (std::regex_search(after, m, ver_re)) {
      findings.push_back(
          {"Error Page Version", "low", base, std::string(sig.product) + " " + m[1].str() + " (leaked via error page)", "", "", ""});
    }
    break;
  }
  ...
  if (srv != resp.headers.end() && srv->second.size() > 2) {
    ...
    if (std::regex_search(srv->second, m, ver_re)) {
      findings.push_back({"Server Header Version", "low", base, "Server: " + srv->second + " (from 404 response)", "", "", ""});
    }
  }
...
std::vector<Finding> scan_feature_fingerprint(const Config&, HttpClient& http, const CrawlResult& crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  for (const auto& [name, val] : resp.headers) header_order += name.substr(0, 3) + ",";
  ...
  if (resp.body.find("/_next/static") != std::string::npos) {
    ...
    if (resp.body.find("__next_f") != std::string::npos) ver = "14+ (RSC flight data)";
    ...
    findings.push_back({"Next.js Fingerprint", "info", base, "Next.js " + ver, "", "", ""});
  }
  ...
  if (std::regex_search(resp.body, rm, react_re)) {
    findings.push_back({"React Version", "info", base, "React " + rm[1].str() + ".x", "", "", ""});
  }

  // Vue.js
  if (resp.body.find("__vue__") != std::string::npos || resp.body.find("Vue.js v") != std::string::npos ||
      resp.body.find("__VUE__") != std::string::npos) {
    std::string ver = "detected";
    std::regex vue_re(R"(Vue\.js v(\d+\.\d+))");
    std::smatch vm;
    if (std::regex_search(resp.body, vm, vue_re))
      ver = vm[1].str() + ".x";
    else if (resp.body.find("__VUE_HMR_RUNTIME__") != std::string::npos)
      ver = "3.x (HMR)";
  ...
  if (resp.body.find("ng-version=\"") != std::string::npos) {
    ...
    if (std::regex_search(resp.body, nm, ng_re))
      findings.push_back({"Angular Fingerprint", "info", base, "Angular " + nm[1].str() + ".x", "", "", ""});
  }

  // Nuxt.js
  if (resp.body.find("__NUXT__") != std::string::npos || resp.body.find("/_nuxt/") != std::string::npos) {
  ...
  // Svelte/SvelteKit
  if (resp.body.find("__sveltekit") != std::string::npos || resp.body.find("/_app/") != std::string::npos) {
  ...
  if (aspnet != resp.headers.end()) findings.push_back({"ASP.NET Version", "low", base, "ASP.NET " + aspnet->second, "", "", ""});
  auto aspnetmvc = resp.headers.find("X-AspNetMvc-Version");
  if (aspnetmvc != resp.headers.end()) findings.push_back({"ASP.NET MVC Version", "low", base, "MVC " + aspnetmvc->second, "", "", ""});
  ...
  if (xrid != resp.headers.end() && xrid->second.size() == 36 && xrid->second[8] == '-') {  // UUID format typical of Rails
    if (resp.headers.find("X-Runtime") != resp.headers.end())
      findings.push_back({"Rails Fingerprint", "info", base, "Ruby on Rails (X-Request-Id + X-Runtime)", "", "", ""});
  }
  ...
  if (cf_ray != resp.headers.end()) findings.push_back({"Cloudflare CDN", "info", base, "Behind Cloudflare", "", "", ""});
  auto cf_cache = resp.headers.find("CF-Cache-Status");
  if (cf_cache != resp.headers.end()) findings.push_back({"Cloudflare Cache", "info", base, "CF-Cache: " + cf_cache->second, "", "", ""});
