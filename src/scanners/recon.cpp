...
std::vector<Finding> scan_api_version_bypass(const Config &, HttpClient &http,
  ...
  const std::vector<std::string> endpoints = {"users", "admin", "config",
                                               "settings", "debug"};
  ...
  for (const auto &ver : versions) {
    for (const auto &ep : endpoints) {
      ...
      if (resp.status_code == 200 && resp.body.size() > 50) {
        findings.push_back({"API Version Bypass", "medium", base + ver + ep,
...
std::vector<Finding> scan_rate_limit_bypass(const Config &, HttpClient &http,
  ...
  const std::vector<std::pair<std::string, std::string>> bypass_headers = {
  ...
  for (const auto &[header, value] : bypass_headers) {
    ...
    if (resp.status_code == 200) {
      findings.push_back({"Rate Limit Bypass", "medium", url,
                          "Bypass via " + header, "", value, ""});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_cloud_metadata(const Config &, HttpClient &http,
                                         const CrawlResult &crawl) {
  std::vector<Finding> findings;
...
std::vector<Finding> scan_firebase(const Config &, HttpClient &http,
                                   const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  ...
  if (fb_resp.status_code == 200 && fb_resp.body != "null" &&
      fb_resp.body.size() > 2) {
    findings.push_back({"Firebase Misconfig", "high", fb_url,
                        "Firebase database publicly readable", "", "", ""});
  }
  return findings;
}

/// Wayback secrets — find leaked secrets in archived pages.
std::vector<Finding> scan_wayback_secrets(const Config &, HttpClient &http,
                                          const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  std::regex secret_re(R"re((?:api[_-]?key|secret|token|password|auth)['":\s]*['"]([a-zA-Z0-9_\-]{16,})['"])re");  std::istringstream stream(resp.body);
  std::string line;
  while (std::getline(stream, line)) {
    ...
    if (std::regex_search(js_resp.body, m, secret_re)) {
      findings.push_back({"Wayback Secret", "high", line,
                          "Potential secret in archived JS", "", "", ""});
      break;
    }
  }
  return findings;
...
std::vector<Finding> scan_ip_header_spoofing(const Config &, HttpClient &http,
                                             const CrawlResult &crawl) {
  std::vector<Finding> findings;
  if (crawl.urls.empty()) return findings;
  std::string base = base_url_from(crawl.urls[0]);
  ...
  const std::vector<std::string> admin_paths = {"/admin", "/internal",
                                                 "/debug", "/status"};
  ...
  for (const auto &path : admin_paths) {
    ...
    for (const auto &[header, value] : spoof_headers) {
      ...
      if (resp.status_code == 200) {
        findings.push_back({"IP Spoofing Bypass", "high", base + path,
                            "Access control bypassed via " + header,
                            "", value, ""});
        break;
      }
    }
...
std::vector<Scanner> register_recon_scanners() {
  return {
      ...
      {"Cloud Metadata", scan_cloud_metadata},
      {"Firebase Misconfig", scan_firebase},
      {"Wayback Secrets", scan_wayback_secrets},
      {"IP Header Spoofing", scan_ip_header_spoofing},
  };
}
