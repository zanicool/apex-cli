...
std::vector<OSINTLeak> OSINTScanner::check_hibp(const std::vector<std::string>& emails) {
  ...
  for (const auto& email : emails) {
    ...
    if (resp.status_code == 200) {
      ...
      while (std::regex_search(body, m, breach_re)) {
        leaks.push_back({"HaveIBeenPwned", "email", email, m[1].str(), "", "high"});
        body = m.suffix();
      }
    }
  }
...
std::vector<EmployeeExposure> OSINTScanner::find_employees(const std::string& domain) {
...
std::vector<TechStackIntel> OSINTScanner::scan_job_postings(const std::string& company) {
  std::vector<TechStackIntel> stack;
  std::vector<std::string> sources = {"site:linkedin.com/jobs \"" + company + "\"", "site:indeed.com \"" + company + "\""};
  std::vector<std::string> tech_keywords = {"AWS", "Azure", "Kubernetes", "Docker", "React", "Python", "PostgreSQL"};
...
std::vector<std::string> OSINTScanner::monitor_news(const std::string& company) {
  std::vector<std::string> mentions;
  std::vector<std::string> queries = {company + " breach", company + " lawsuit"};
...
std::vector<TechStackIntel> OSINTScanner::scan_infrastructure(const std::string& domain) {
